...
Une image déposée est **reprise telle quelle** si elle tient déjà dans 128 px : jamais agrandie, jamais lissée. Un pixel art agrandi — a fortiori en lissant — perd exactement ce qui en fait du pixel art. Seules les photos d'appareil (plus de 128 px) sont réduites — à 64 px, recadrées au carré, en JPEG depuis le chantier 8 — et là le lissage sert puisqu'il s'agit de photos. Le dessin, lui, se fait toujours en pixels francs.
...
## 58. Chantier 8 — l'atelier honnête

**L'établi Amis produit exactement ce que le jeu contient, et le contenu ne pèse plus trois fois son poids.** Mesure de référence : ouvrir l'établi, exporter sans rien toucher, recoller, rebâtir → `dev/content5.js` identique octet pour octet ; `index.html` passe de 1 241 à 1 092 Ko (−149 Ko).

- **`content5.js` change de forme** : `FRIEND_IMAGES` (photos), `FRIEND_SHEETS` (planches), puis **`FRIEND_CONTENT = { characters, pets, pairs }`** poussé dans `CONTENT`. Le fichier n'est plus « remplacé en entier » à l'aveugle : l'établi le **relit à l'ouverture** (`amisBase()` : chaque définition devient une fiche par `defToEntry`) et l'export **fond** les changements locaux dedans. Les trois copains, les quatre animaux et les trois duos de l'auteur apparaissent donc dans l'établi, planches comprises.
- **Une fiche porte `def`**, la définition telle que le jeu la lit ; `petDef` / `charDef` (80_atelier.js) reposent le formulaire par-dessus et ne touchent à rien d'autre : `hidden`, `duo`, `fly`, `duoName`, `hp`, `revive`, un caractère importé (`trait: 'importe'`) traversent intacts. Une fiche est « locale » quand sa signature (définition + images + planches) diffère de celle du fichier ; seules celles-là sont réenregistrées dans `CONTENT` (en place, sans changer l'ordre) et gardées dans le navigateur. Les animaux ont maintenant leurs **planches** (repos, marche, attaque, sonné) et leur ligne « ce qu'il fait ».
- **Le texte exporté est écrit par `Amis.snippet`** (78_amis.js, sans DOM, chargeable depuis node) avec `Amis.serialize`, un imprimeur de littéraux qui suit les règles de Prettier (140 colonnes, guillemets simples, virgules es5, un tableau de plusieurs objets cassé ligne à ligne) : `prettier --check` n'a rien à reformater. Ordre des planches : les copains puis les animaux, chaque clip dans l'ordre de son `anim`.
- **PNG palette** : le canvas n'écrit que du RGBA ; `Amis.pngRaw` / `pngAssemble` / `encodeCanvas` écrivent un PNG de type 3 (PLTE + tRNS, palette dans l'ordre d'apparition, déflate par `CompressionStream`). Les 31 planches livrées, toutes sous 64 couleurs, passent de **261 à 86 Ko de base64**, mêmes pixels (vérifié pixel à pixel). Une planche ou une image déposée est réencodée ainsi à l'import ; au-delà de 256 couleurs, PNG RGBA.
- **Photos** : au-dessus de 128 px c'est une photo, réduite à **64 px** (`PHOTO_MAX`, comme le dit CLAUDE.md), lissée, en **JPEG** à 85 % ; jusqu'à 128 px l'image est reprise telle quelle en PNG palette. `IMG_MAX` garde son sens de seuil.
- **Une seule copie de chaque image** : `way.props.custom` n'existe plus (`Sprites.addCustom` ne réécrit plus tout le JSON à chaque image, `loadCustoms` est retiré) ; les planches du fichier sont lues **une fois** au démarrage (`Sprites.loadFriends`), l'établi n'ajoute que ce qui a changé. Un accessoire de décor déposé dans l'établi rythme vaut jusqu'au rechargement (le ranger dans `assets/sprites/pixel/` pour le garder).
- **IndexedDB** (`way_atelier`, magasin `kv`, clé `amis`) remplace `localStorage.way_amis_v1` (repris une fois puis effacé) et ne contient que le local : fiches changées, nouvelles, `deleted`, `pairs` si modifiés ; vide, la clé est supprimée. **Refus explicite** avant dépassement : `placePour(octets)` lit `navigator.storage.estimate()` et refuse l'image avec le message « Plus de place dans le navigateur (x Mo sur y) : exporte… » plutôt que de la perdre à la sauvegarde.
- **Décor animé** ([12]) : la décision est « livrer » — l'établi Animations reste dans le build, les quatre salles cadencées du biome 1 se construisent au chantier 9.

Test : `allerretour.js` — 11 mesures (l'export identique au fichier, point fixe de Prettier, l'établi qui montre les amis du fichier, les 31 planches palette découpées comme avant, l'encodeur pixel-exact et plus léger, une fiche modifiée dans le jeu / IndexedDB / l'export à côté des amis du fichier, la survie au rechargement, photo → JPEG 64 px et pixel art → PNG 32 px, le refus quand le navigateur est plein, un animal supprimé qui sort avec son duo). `amis.js`, `export.js`, `sprites32.js`, `vues.js` suivent (les fiches du fichier viennent d'abord, la remise à zéro passe par `Atelier.amisReset()`).

## 59. Chantier 9, première séance — quatre biomes, quatre structures

**Chaque biome a son ordre de salles, sa salle unique, ses coffres offerts et un boss de salle 9 qui ne rejoue pas la salle 5.** Les comportements neufs par archétype et les quatre salles cadencées viennent aux séances suivantes.

- **Les ordres** (types, `Content.roomsOf`) :
  - ADMISSION : préparation · aléatoire · combat + pièges *(coffre)* · **le sous-sol** · mini-boss · modulaire · tempo *(coffre)* · combat + pièges + modulaire · revanche ;
  - LA SERRE : préparation · combat + pièges · aléatoire *(coffre)* · modulaire · mini-boss · tempo · combat + pièges + modulaire *(coffre)* · **le pont** · revanche ;
  - LA CONCESSION : préparation · **le train** · combat + pièges *(coffre)* · aléatoire · mini-boss · combat + pièges + modulaire · modulaire *(coffre)* · tempo · revanche ;
  - LE SÉRAIL : préparation · aléatoire · modulaire *(coffre)* · combat + pièges · mini-boss · **le bazar** *(coffre caché)* · tempo · combat + pièges + modulaire · revanche.
- **Plus de salle-coffre** ([14]) : les types `CHEST` / `CHEST_FINAL` ne sont plus joués (les 4 et 8 sont des combats, le type `COMBAT_TRAP_MODULAR` sert enfin). Une salle porte `chest: true` : à la salle sécurisée, `Room.offerChest()` pose le coffre devant la porte (tuile 19,6, à gauche du cœur) ; `chestHidden: true` (le bazar) le pose sur une tuile libre tirée loin de la porte (x ≤ 15), avec le message « Un coffre, quelque part entre les étals ». Le point de sauvegarde (`Run.checkpoint`) reste en fin de 4 et 8.
- **Les salles uniques** (types `SOUS_SOL`, `PONT`, `TRAIN`, `BAZAR`, libellés dans `ROOM_TYPES`) : le **sous-sol** du biome 1 (salle 4) est un couloir de piliers qui impose le défi « lumières coupées » ; le **pont** de la Serre (salle 8) est une passerelle de trois tuiles (`terrain` : eau `~` de part et d'autre, traversable à 72 %) prise des deux côtés ; le **train** de la Concession (salle 2) fait traverser trois wagons qui ne s'arrêtent pas (`slide_wall` avec `loop: true` : le mur va d'un trait et repart du début, 36_modular.js) ; le **bazar** du Sérail (salle 6) est un damier de 23 étals où le coffre se cache.
- **Le défi suit le type** (38_challenges.js) : `COMBAT_CHALLENGE` et `SOUS_SOL` en ont toujours un (le sous-sol : `lights`), `COMBAT_MODULAR` et `COMBAT_TRAP_MODULAR` à 60 %, plus de liste de numéros.
- **Les salles modulaires diffèrent** : Admission murs coulissants + rotor + sol cyclique ; Serre deux rotors contraires + sol ; Concession trois barrières de corral décalées + sol ; Sérail deux tentures en travers + rotor à trois pales.
- **Le boss de la salle 9** : `revenge.phases` remplace les phases du mini-boss (32_enemies.js, `Boss`) ; les phases supplémentaires de la revanche restent (trois phases au total), la plaque et le mimétisme aussi. Portier rév. B : laser, charges, anneau, puis charge / onde / éventail / mèches ; Serriste : ronces, spirale inverse, spores, puis laser / éventail / moucherons / frappe ; Marshal : duel, anneau, charge, puis mines / spirale / éventail / ruée ; Vizir : mirages, anneau, frappe, puis tempête / spirale / charge / cobras.
...
## 61. Chantier 9, troisième séance — les salles cadencées et la performance

**Le décor animé est joué, le défi des lumières et le flash des ennemis ne coûtent plus ce qu'ils coûtaient, le train a ses wagonnets.** Fin du chantier 9.

- **Quatre salles cadencées** ([12], content.js, `anims` des salles 1, 2, 3 et 6 de l'Admission) : le carrelage de l'accueil s'allume en damier sur chaque temps (`tile_color`, mode `checker`, deux teintes) ; en salle 2 une onde part du centre sur le temps fort et une couronne de dalles se soulève un temps sur deux (`ring`, `tile_lift` sur une liste de cases) ; en salle 3 un balayage traverse la salle à chaque mesure dans le sens du tourniquet (`tile_color`, mode `sweep`, deux mesures) ; en salle 6 quatre ventilateurs tournent d'un quart de tour par temps et une valve saute sur les temps forts (`spinner`, `bouncer`, avec les accessoires `computer-fan` et `valve`). Chaque partition pose aussi ses **néons** (`light`) : alternés bleu / orange sur le temps fort et le troisième temps — `Room.placeLights` s'efface quand la salle en déclare. Le format est celui que l'établi Animations exporte (`{ kind, x, y, w, h, params, beats: { bars, hits, active } }`, `cells` pour un motif peint case à case) : ce qui sort de l'atelier se colle tel quel.
- **Le masque des lumières coupées** ([29], 38_challenges.js) : le dégradé radial est **dessiné une fois** dans un canvas de 256 px (`lightDisc`) et collé à l'échelle de chaque faisceau ; le masque n'est **refait qu'une image sur deux** (`MASK_EVERY`) — il vit en coordonnées du monde, l'image intermédiaire reste juste. Banc A/B (rendu logiciel, les deux builds dans le même navigateur, écart « avec le défi − sans », phase d'accueil à deux disques) : **1,28 → 0,97 ms** sur la page la plus lente ; avec les faisceaux, `cadence.js` mesure 1,45 ms (cible : sous 2 ms). Le flash de douze ennemis passait 0,5 ms au banc, il ne se distingue plus du bruit. `c.maskBuilds` compte les reconstructions pour le test.
- **Le flash blanc précalculé** ([41], 15_sprites.js) : `flashFrame` garde en cache une image blanchie par (sprite, image, taille, intensité), 160 entrées au plus ; `Sprites.draw` et `drawSheet` la collent d'un `drawImage` au lieu de redessiner le sprite dans un canvas intermédiaire à chaque image. Les teintes de statut (brûlure, poison) gardent le chemin direct. `Sprites.flashCacheSize()` pour les tests.
- **Les wagons du train** (36_modular.js) : un `slide_wall` avec `look: 'wagon'` dessine un wagonnet (`mine-wagon`) par tuile sur un rail pointillé qui traverse la salle, retourné selon le sens, avec un liseré orange quand il va entrer — plus de cadre bleu générique.
- **Le bot apprend le pas de côté** (70_debug.js) : sans progrès vers son objectif pendant 1,2 s, il part franchement de côté 0,7 s au lieu d'osciller contre un pilier — le sous-sol et le bazar le coinçaient (`choix.js` expirait en salle 4).
- **Ce qui reste du chantier 9** : [24] le terrain de salle (étendu ou retiré) — non tranché, à décider avec l'auteur ; les partitions des trois autres biomes se posent dans l'atelier quand on le voudra (le format est prêt).

Test : `cadence.js` — 6 mesures (quatre salles à partition, le pouls d'une dalle qui monte et retombe avec les néons de la partition, les dalles qui dessinent une surface, les wagonnets, le masque refait 10 fois pour 20 images et sous 2 ms, le cache du flash rempli une fois).

## 62. Chantier 7 — se comparer entre amis

**Une raison de rejouer : un score, la même partie pour tout le monde un jour donné, une ligne à coller à un ami, et une sauvegarde qui voyage.**

- **Le score d'une partie** (`Progression.runScore`, 20_progression.js) : chaque salle sécurisée vaut 400 + 600 × sa qualité (les deux salles de boss comptent double), chaque salle atteinte 200, chaque niveau 150, et une victoire ajoute 6 000 − 4 × les secondes (jamais négatif) ; arrondi à la dizaine. Une victoire bien jouée tourne autour de 15 à 20 000 points, une mort en salle 3 autour de 2 000. `Run.score(win)` le calcule sur la partie en cours, `Run.summary(win)` fige le résumé (`score, room, win, time, seed, biome, char, pet`) au moment de la mort ou de la victoire, avant `Meta.recordRun(win, résumé)`.
- **La graine** : chaque partie en a une, notée dans `G.run.seed`. `Run.start` la prend dans l'ordre : donnée (bot, atelier) ; sinon `profile.seedNext` (une ligne d'ami collée, jouée une fois puis oubliée) ; sinon la **graine du jour** si `profile.dailySeed` est coché (`Meta.dailySeed()` : la date locale en chiffres, 20260911) ; sinon le hasard. `RNG.reseed(seed)` en début de partie : mêmes paires bonus/malus, même défi en salle 2, mêmes tirages. `VFX_RNG` reste libre (les particules ne comptent pas).
- **La ligne de résultat** (`Run.resultLine`) : « WAY · Martin + Uno · ADMISSION · salle 9 (fini) · 4 min 12 · 18 430 pts · graine 20260911 ». L'écran de fin l'affiche avec un bouton **Copier le résultat** (presse-papiers, sinon la ligne reste sélectionnable) ; le camp a une case **« colle ici la ligne d'un ami »** : `Run.parseResult` relit la graine (et le palier s'il est nommé), pose `seedNext`, montre « Prochaine partie : graine N » avec une croix pour oublier.
- **Les dix meilleures parties** par personnage (`profile.best[charId]`, `Meta.recordScore`) : triées par points puis salle puis temps, plafonnées à dix ; le rang de la partie qui vient de finir est dans `G.run.rank` (1 = record) et l'écran de fin le dit (« nouveau record », « 3ᵉ meilleure partie »). Le camp les liste sous les trois questions (rang, points, où, temps, palier, compagnon, date, graine) — cliquer une graine la rejoue.
- **La sauvegarde qui voyage** (volet **Sauvegarde** dans l'en-tête du camp) : `Meta.exportText()` donne `{ way: 'sauvegarde', v, at, profile }` — toujours le profil Normal, jamais celui du mode test ; **Télécharger** (fichier `way-sauvegarde-AAAAMMJJ.json`), **Copier**, **Importer un fichier**, ou **coller un texte**. `Meta.importText` refuse ce qui n'est pas une sauvegarde WAY, garde une copie de l'ancienne sous `way_save_secours_import`, passe la nouvelle par les mêmes migrations que le chargement et l'écrit en `way_save` (même depuis le mode test). Sauvegarde **v4** : `best`, `dailySeed`, `seedNext` arrivent par la fusion (SCHEMA.md §8).

Test : `comparer.js` — 8 mesures (la formule du score, la graine reproductible et distincte, la graine du jour cochée dans le camp, l'écran de fin avec le score / la graine / la ligne et son bouton, la ligne collée qui rejoue la graine et le palier une fois, le tableau trié plafonné à dix, l'export qui se relit et l'import d'une v3 migrée en v4 avec sa copie de secours, le volet Sauvegarde du camp).

## 63. Chantier 10 — le téléphone, partie performance

**Le jeu ne sait pas ce qu'il coûte sur un téléphone sans le mesurer sur place : il se mesure lui-même, se met en économe quand il ralentit, et les salles à pièges et à modules ne posent presque plus de flou.**

- **La mesure** (`Perf`, 00_core.js, appelée par la boucle d'`Engine`) : les images par seconde de la dernière seconde (`Perf.fps`), le pire des dix dernières (`Perf.min`), le temps moyen passé dans `render()` (`Perf.renderMs` — les ordres, pas la rastérisation qui vient après), et le nombre de **flous** (`shadowBlur > 0`) posés par image (`Perf.blurs`) : le réglage `shadowBlur` du contexte est intercepté (`Perf.hook`), chaque flou coûte une passe de plus par forme. `Perf.line()` donne une ligne à coller : « WAY perf · 54 i/s · min 50 sur 10 s · rendu 0,7 ms · 1 flous · complet · salle 2 · 900×420 ×2 tactile · Linux; Android 13; Pixel 7 » ; le rapport (`Rapport.texte`) la reprend.
- **Le compteur** : en bas au centre (entre le stick et les boutons au tactile), vert, rouge sous le seuil ; coché dans la pause (« Compteur d'images par seconde », `profile.perfShow`) ou `?perf` dans l'adresse.
- **Le rendu économe** (`Perf.setEco`) : les flous ramenés à 0 à la source (mais toujours comptés — la mesure reste honnête), la densité de pixels plafonnée à 1 (`Engine.resize` : un pixel physique par pixel CSS, quatre fois moins de pixels sur un téléphone ×2), et `body.eco` retire le flou d'arrière-plan des écrans (`backdrop-filter`, qui se recalcule à chaque image du canvas sous lui). **Automatique** (`profile.perfMode` 'auto', le défaut) : au tactile seulement, en jeu, hors pause et onglet visible, moins de 45 i/s pendant 3 secondes consécutives l'allument pour la session, avec un mot à l'écran ; 'eco' l'impose, 'full' l'interdit — le réglage **Rendu** de la pause. Les boutons tactiles (`.tbtn`) n'ont plus jamais de flou d'arrière-plan.
- **Les flous qui restaient** : les lasers (balayage, rotor, grille, rayon), les tourelles, la boule de feu murale, la scie, les murs coulissants, les chaînes et le pivot du rotor, les dalles annoncées, la zone sûre, les anneaux du tempo, la zone du défi capture et ses interrupteurs, les yeux dans le noir, la porte ouverte, les flaques, les leurres, les balises et les lames de mêlée passent par `Halo` : `Halo.line` et `Halo.rect` (nouveaux, même recette que `Halo.ring` : un trait large et pâle sous le trait net, le tiret en cours ne s'applique qu'au trait net) et `Halo.draw` pour les disques. Par image : salle 3 → 1 flou (2 avant), salle 6 → 5 (11), salle 8 → 3 (11) ; ce qui reste vient des ennemis (télégraphie, flash), du boss, des rayons et des textes ponctuels. Sur la machine de test (rastérisation logicielle), `Room.render` gagne 0,15 à 0,5 ms par image selon la salle. **Mesure de l'auteur (12 septembre 2026)**, téléphone Android 10, Chrome 153, écran 832×384 ×3,8 : « 60 i/s · min 60 sur 10 s · rendu 1,8 ms · 3 flous · complet · salle 1 » — l'objectif (≥ 50) est tenu sans rendu économe.

Test : `perf.js` — 11 mesures (Halo.line/rect, plus de shadowBlur dans les rendus des pièges, modules, tempo, porte et lames, au plus 7 flous par image en salles 2/6/7/8, la mesure qui vit et sa ligne dans le rapport, le rendu économe qui coupe et se remet, l'automatique au tactile après 3 s lentes et jamais au clavier, la pause, le compteur dessiné, les boutons tactiles sans flou, `?perf`).

## 64. Retours de l'auteur après le chantier 10 — le zoom qui restait et la musique qui ne repartait pas

**« Les niveaux de la Serre sont trop zoomés, on ne voit même pas la porte à droite » ; « la musique ne se lance pas quand on relance une partie ».** Deux restes de scènes (F-6) qui survivaient à la partie d'avant.

- **Le zoom de scène** : la mort zoome à 1,3 (`Camera.zoomTo(1.3)`), la victoire à 1,15, et rien ne les remettait à 1 — la partie suivante (« Repartir tout de suite », ou le palier suivant depuis le camp, donc la Serre après une victoire dans l'Admission) se jouait avec ce zoom en plus. `Camera.resetScene()` (point à regarder, zoom de scène, impulsion) est appelé à chaque entrée de salle. Et `Camera.clamp` cadrait avec le seul réglage de zoom, sans le zoom de scène : la vue pouvait dépasser la salle d'un côté et couper la porte de l'autre. Le cadrage, `apply` et `toWorld` (la visée à la souris) passent par `Camera.effectiveZoom()` = réglage × impulsion × scène.
- **La musique** : à la mort, `Music.dying` ralentit la bande 2,2 s puis l'arrête et oublie la piste courante. Une partie repartie pendant ces 2,2 s demandait la même piste (`play('biome')` → même clé → « elle joue déjà »), puis la rampe finissait et coupait tout : silence jusqu'à la salle du boss. `Music.play` sait maintenant qu'une bande meurt (`st8.dying`) : il annule la rampe, remet la vitesse à 1 et repart.

Test : `reprise.js` — 7 mesures (le cadrage avec un zoom de scène 1,3 tout à droite : la porte dans l'écran et rien au-delà de la salle ; le zoom de scène remis à 1 à l'entrée de salle ; la mort qui zoome à 1,3 puis « Repartir tout de suite » au zoom 1 ; la musique qui joue dans la partie repartie ; `Music.play` pendant la mort de la bande).

## 65. Chantier 11 — la dette, en une séance

**Le ménage prévu « au fil de l'eau », fait d'un coup : les trois grandes machines à états deviennent des tables, les valeurs par défaut disent la vérité, les nombres de la difficulté ont un nom, et les restes de la phase 1 partent.**

- **Trois tables kind → fonction** : `BOSS_PATTERNS` (32_enemies.js, 17 motifs : ring, fan, spiral, charge, slam, duel, quake, mines, roots, sandstorm, mirage, summon, laser_sweep, teleport, shield, slow, pull), appelée sur le boss avec `(c, dt, dur, pl, rate)` — `Boss.runPattern` ne garde que la télégraphie et l'aiguillage ; `PET_ACTIONS` (31_pets.js, 9 comportements, `(dt, tick, pl)`) — `Pet.update` garde le repos, le KO, les scènes et la marche ; `UPDATERS` (38_challenges.js, 5 défis, `(room, c, dt, pl)`). Un kind inconnu : le boss finit son motif, le compagnon suit, le défi ne fait rien. Un nouveau motif ou comportement s'ajoute en une entrée de table, sans toucher à la machine.
- **Valeurs par défaut honnêtes** (`Content.character`, `Content.biome`) : sans id, le premier (c'est ce qu'un profil neuf attend) ; un id inconnu (contenu retiré, ligne d'un ami avec un palier qu'on n'a pas) donne `null` et un mot dans la console au lieu de faire passer le premier personnage pour le bon. Le camp, `Run.start` et la ligne collée se rabattent explicitement.
- **Les nombres du chantier 4** dans `05_balance.js` : `difficulty { speedBase 0,7, speedPerD 0,3, fireBase 0,75, firePerD 0,25 }`, `timerBonus 30`, `enrageMul 1,3`.
- **Les restes** : en-têtes « WAY — » partout, plus de `deepClone`, de Gamepad « prévu », de `slowFactor`, de marqueur `TODO_SPRITE` (le repli Canvas est nommé comme tel).

Non fait, et pourquoi : `Atelier.tune` (225 lignes) n'est pas un `switch` mais un formulaire par famille ; le critère « aucune fonction de plus de 60 lignes » est abandonné (voir PLAN-CHANTIERS.md, chantier 11).

Test : `dette.js` — 5 mesures (les tables couvrent tout le contenu et il ne reste pas de `switch`, les valeurs par défaut, les nombres dans BALANCE, plus de restes dans `dev/`).

## 66. Chantier I-9 — les menus en paysage court

**Sur un téléphone tenu à l'horizontale, le camp et la prépa se lisent en deux écrans au lieu de quatre, sans qu'un pixel ne bouge au bureau.**

- **Le diagnostic** (photos de l'auteur, 832×384 ×3,8) : `--ui-scale` = min(l/1280, h/720) ≈ 0,53 réduisait toutes les **dimensions** des cartes de moitié alors que les **polices** ont un plancher (`max(13px, …)`) — du texte de bureau empilé dans des cartes de poche (la phrase du palier sur 7 lignes, la carte de palier à 63 % de la hauteur visible). Rien ne distinguait le paysage court du portrait : la mise en une colonne d'I-8 gaspillait 75 % de la largeur. L'en-tête et le bouton PARTIR mangeaient 145 px sur 384.
- **`--menu-scale`** = `max(0.72, var(--ui-scale))` (style.css, fin de fichier) : les 34 dimensions en `px × échelle` des blocs `.hub3` et `.prep2` (cartes, portraits, icônes, grilles) passent par ce plancher ; les marges du conteneur restent à `--ui-scale`. Au bureau l'échelle vaut 1 : rien ne change.
- **Le paysage court** : `@media (max-height: 500px) and (orientation: landscape)` — l'en-tête sur une ligne (« WAY · crédits · boutons », le « Camp de base » et l'intercom cachés), le bouton PARTIR sur une ligne de 44 px (le récapitulatif de l'équipe passe à droite du titre), le camp sur **deux colonnes** (« Qui part ? » à gauche, « Avec qui ? » à droite, la carte d'équipe et « Où ? » en pleine largeur ; les rangées en `flex-wrap` mais toujours `overflow-x: auto`), les titres de section collants, les cartes de compagnon sans leur ligne de description (la carte d'équipe la dit), la **phrase du palier sous la rangée** (`.lvltag`, rendue par `showHub` et cachée partout ailleurs) au lieu d'être dans la carte, les puces bonus ⇄ malus fines (elles ne sont pas cliquables) tandis que la graine du jour et la ligne à coller gardent 44 px. En prépa : l'en-tête collant et compact, les paires **côte à côte** (le plafond `body.touch .cards .card { max-width: 15em }` levé pour `.pairpick`), les compétences sur une rangée, les armes en `minmax(118px, 1fr)`.
- **Mesuré en 900×420 tactile** : le camp de 3,7 à 2,0 écrans de défilement, la prépa de 3,2 à 2,2 ; cartes de personnage 58 → 81 px, paires 179×234 → 406×71, cartes de palier 189 → 88 px de haut, en-tête 62 → 50 px, bouton 58 → 44 px.

Retour de l'auteur après coup : choisir une arme ramenait en haut de la prépa (`render()` reconstruit tout le panneau) — la position de défilement est gardée et restaurée à chaque reconstruction, comme au camp.

Pas fait, à dessein : la prépa en étapes horizontales (onglets), qui demande du JavaScript et de revoir `prepa.js` / `choix.js` — les rangées suffisent. Le toucher sur une puce bonus ⇄ malus qui montrerait la phrase reste une finition possible.

Test : `paysage.js` — 12 mesures (le défilement gardé quand on choisit une arme, le plancher d'échelle, le camp en moins de 2,5 écrans, les deux colonnes, aucune carte deux fois plus haute que large, l'en-tête et le bouton en une ligne avec l'équipe, la phrase du palier sous la rangée, les puces fines et la graine à 44 px, les paires côte à côte et basses, les compétences en rangée, la prépa en moins de 2,5 écrans avec son en-tête collant, et le bureau inchangé).

## 67. Chantier 12, séance A — les scènes

**Chaque salle des quatre paliers a maintenant une silhouette voulue : ses accessoires et son décor sont posés à la main, la Serre a ses propres pièges, les dix pièges jamais posés entrent en jeu, et les quatre salles du tempo n'ont plus la même partition.** Contenu seulement (`dev/content*.js`) ; le moteur ne change pas. Le diagnostic et le détail sont dans `PLAN-VARIETE.md`.

- **Des kinds à la main partout** (`obstacles[].kind`) : l'Admission en casiers, cuves, perfusions, poubelles, réservoir ; la Serre en jardinières, fontaines, souches, buissons, fioles, plante carnivore ; le train et la poudrière de la Concession en caisses, tonneaux et fûts ; le bazar du Sérail par allée (auvents, paniers, jarres, vases) et braseros en salles 5, 7, 8. `Room.dress` ne tire plus au sort que là où rien n'est dit — plus nulle part dans ces 36 salles.
- **Du décor posé à la main** (`deco[]`, 2 à 7 par salle) : croix, panneaux, valves, piles et fioles à l'hôpital ; fleurs, semis, champignons, feuilles, pots à la serre ; affiches wanted, crânes, portes de saloon à la Concession ; tapis, lanternes, épices, théières, gemmes, encens au Sérail. Une salle qui déclare son `deco` n'est plus semée.
- **Les pièges de la Serre** (10 définitions, `content2.js`, `trapPool` remplacé) : Lianes (rotor à 3 bras), Épines, Spores, Sève brûlante, Arroseur (balayage), Treillis, Plante cracheuse, Brumisateur, Lampe de culture (rayon), Tondeuse (scie sur rail) — mêmes mécaniques, noms, couleurs et descriptions à elles ; les neuf salles les emploient. Les pièges de l'Admission ont chacun une `color` (bleu néon, orange sodium, gris acier, vert pâle) : plus de rouge partout.
- **Les pièges orphelins posés** : Admission — scies sur rail en salle 3 (le bloc opératoire) et 8 (le couloir des brancards), gyrophare au centre et diffuseur en salle 7 ; Serre — l'arroseur qui balaie les rangs de buissons de la salle 2, le brumisateur et les lianes en mesure en salle 6 ; Concession — revolvers muraux et porte de saloon en salle 4, barbelés et fil de détente dans la poudrière (salle 6), gatling au centre de la salle du tempo ; Sérail — lanterne à braises en salle 2, rai de soleil vertical le long des claustras en salle 4, derviche de lames au centre du tempo, braseros roulants en salle 8, deux rais de soleil en travers de la salle 9.
- **Quatre partitions de tempo distinctes** : gyrophare et diffuseur (Admission), brumisateur et lianes (Serre), gatling (Concession), derviche (Sérail) ; le reste de chaque partition garde ses pièges de coin, ses bouches et ses tireurs.
- **Des lumières à la couleur du palier** (`anims` `light`) : orange de secours en salle 9 de l'Admission, ambre lent en salle 9 de la Serre, or et violet en salle 1, orange des braseros en salle 5, violet en salle 9 du Sérail.
...
## 69. Chantier 12, séance C — ce qui bouge et les accessoires

**Le décor bouge dans les quatre paliers, les murs coulissants sont habillés, chaque palier reçoit des accessoires découpés des packs, l'hôpital et la serre ont leurs ennemis-objets, et chaque salle porte cinq à dix décorations posées à la main.**

- **Le décor animé hors de l'Admission** (`anims`, format de l'établi Animations) : la rivière de la Serre a un papillon (`mover`) qui saute de point en point sur le temps fort, ses cuves deux fioles qui bouillonnent (`bouncer`), son pont trois grenouilles ; le moulin de la Concession tourne d'un quart de tour par temps (`spinner` `windmill` ×2,2) pendant qu'un vautour (`mover`) fait le tour de la salle ; le tempo du Sérail a deux sabliers qui se retournent à chaque mesure (`spinner` `sands-of-time`, pas de π). Les lumières de la séance A restent.
- **Les murs coulissants habillés** (`36_modular.js`) : `look` accepte n'importe quel accessoire, un par tuile le long du mur (horizontal ou vertical) ; `look: 'wagon'` garde son rail. Les tentures du Sérail (`theater-curtains`, salle 3), les barils de la Concession (`cellar-barrels`, salle 7), les lianes de la Serre (`vines`, salle 7).
- **29 accessoires nouveaux en PNG** (`assets/sprites/pixel/`, 45 fichiers avec les variantes, `node dev/index-pixel.js`), découpés des planches CC0 Kenney Roguelike Indoors et Ninja Adventure par un script Pillow : hôpital — lit, comptoir, machine, tabouret, tuyauterie, chaudière ; serre — buisson rond, arbre vert, arbre rose, gros rocher, petit rocher, souche, fleurs, champignons, pot moussu ; Sérail — grand palmier, sphinx, lion, puits, étal rouge, étal jaune, tapis, tapis vert, ossements, buisson sec, pot rond ; Concession — saguaro, arbre mort, tas d'os. Dans `PROP_DEFS` ils portent `png: true` (rien à rastériser), `BLOCK_KIND` / `DECO_KIND` leur donnent un kind, `BLOCK_GROW` fait déborder arbres, palmiers, statues, `Room.DRESS` les connaît. Posés dans les salles : comptoirs de la salle d'attente, machines et chaudières, arbres de la rivière, souches et rochers, saguaros de la salle 1, étals rouges et jaunes du bazar, palmiers de la cour, sphinx et lions de la revanche.
- **Les ennemis-objets de l'hôpital et de la serre** (`SPRITE_DEFS`) : la Nuée devient des rats (`pets/rat`), l'Éclipse un œil qui traverse (`all-seeing-eye`), la Spore un champignon qui roule (`grass-mushroom`, `roll`), les Moucherons des papillons (`butterfly`) — le même chemin que le baril, le scorpion et le crotale de la Concession.
- **« N'hésite pas à mettre du décor »** (l'auteur, pendant la séance) : deux passes de décorations de plus par salle (cinq puis sept, « plein de décor »), tirées d'une palette par palier et posées par un script sur des tuiles libres (hors obstacles, apparitions, pièges, modules, animations, colonnes 0-2, couloir de la porte, eau et pont ; jamais deux côte à côte), un cinquième en grand ; le résultat est écrit dans le contenu, donc figé. 420 décorations ajoutées, 523 en tout : chaque salle en porte douze à dix-neuf, le pont de la Serre garde ses grenouilles seules.

Tests : `biomes.js`, `comportements.js`, `sprites32.js`, `vues.js`, `palette.js`, `spawncheck.js` passent ; batterie complète.

## 70. Chantier 12, séance D — le terrain

**Huit salles de plus ont un relief : des guichets qu'on ne franchit qu'en ruée, des flaques et des bassins où l'on ralentit, du terreau et du sable où l'on s'enlise, des rideaux de vapeur qui coupent la vue. Un vérificateur hors ligne et un test de la batterie garantissent que chaque salle reste traversable, et la galerie des 36 salles est livrée.**

- **Le terrain hors du Pont** (plans ASCII 13×24 dans `dev/content*.js`, un mot de scène au-dessus de chaque plan) : l'Admission — salle 1 deux guichets en travers (`n`, les balles passent, pas les pas, la ruée les saute), salle 2 quatre bancs de salle d'attente autour desquels on tourne, salle 4 trois flaques qui brillent dans le noir (`~`, vitesse ×0,72) ; la Serre — salle 3 quatre carrés de terreau frais (`,`, ×0,55 ; les Ronces qui bondissent l'ignorent), salle 4 deux bassins autour des bras d'agitation (dans l'eau on est lent, sous le bras on est frappé), salle 7 deux rideaux de vapeur (`:`, la vue ne passe pas, tout le reste si) qui laissent le centre ouvert ; la Concession — salle 4 deux langues de sable mouvant ; le Sérail — salle 3 quatre bassins d'eau tiède aux coins du hammam. Douze salles sur 36 ont un plan, contre quatre avant. Les palettes de terrain manquantes sont ajoutées (`palette.terrain` : muret et eau pour l'Admission, boue et vapeur pour la Serre, boue pour la Concession, eau pour le Sérail). Les décorations qui tombaient dans l'eau ou sous un muret sont retirées ; le tas de crânes de la salle 3 de la Concession qui dormait dans la roche est déplacé.
- **Le vérificateur hors ligne** (`node dev/check-terrain.js`) lit maintenant le bloc de chaque salle et compte ses `obstacles` comme solides dans le parcours entrée → porte ; il exige de plus un **couloir de deux tuiles de large** d'un bout à l'autre (fenêtre 2×2 libre propagée en largeur), murets franchissables en ruée mis à part : leurs brèches d'une tuile sont voulues (les terrasses du Sérail). Douze plans, zéro erreur.
- **Le test `salles.js`** (batterie) crée les 36 salles sans les lancer et mesure : la porte atteignable à pied (terrain + obstacles) et par un couloir large ; le sas d'entrée (colonnes 0-2, lignes 5-7) et le couloir de la porte (21-23) libres de tout obstacle, terrain solide et piège (une grille de tempo qui couvre la salle est ignorée : elle prévient) ; un carré 3×3 libre là où le boss apparaît (colonne 17) ; chaque décoration sur du sol nu, jamais dans un obstacle ni sur du terrain ; au plus 40 rectangles d'obstacle par salle (le maximum réel est 23, au Sérail). `spawncheck.js` garde les apparitions fixes et les pièges.
...
## 72. Chantier 13, séance A — le socle des pièges

**Le moteur des pièges est réécrit en trois tables sans changer une pose ni une définition, ses dettes sont payées, et deux tests nouveaux garantissent ce que personne ne mesurait : qu'un piège blesse, et qu'un chemin sûr existe toujours.**

- **Trois tables** (`dev/34_traps.js`, 927 lignes contre 877) : `TRAP_TRIGGERS` (quand — `cycle` : période ou partition, `run` : une annonce puis toujours en marche, `shot` : des coups à un instant), `TRAP_BODIES` (où et à quoi ça ressemble — `sweep`, `arms`, `grid`, `mouth`, `tiles`, `cloud`, `rail`, `turret`, `emitter`, `beam`, chacun avec `hits`, `danger`, `render` et pour les tireurs `every`, `proj`, `angles`), `TRAP_EFFECTS` (quoi — `damage`, `gas`, `shoot`, `salvo`). `TRAP_LEGACY` traduit les dix `kind` historiques en triplets ; une définition peut nommer `trigger`, `body`, `effect`, `snd` elle-même (SCHEMA.md). La classe `Trap` garde l'horloge (cycle, partition, coups, `syncBeat`) et devient l'assembleur : `update` demande l'étape au déclencheur, laisse le corps se mettre à jour (visée, grincement), annonce, tire ou frappe par l'effet. Un déclencheur est pur : le bot l'appelle avec un temps futur. Le `switch` d'adaptation des paramètres est remplacé par des conversions génériques (tuiles → pixels, mots → nombres).
- **Les dettes payées** : `syncBeat` calculé une fois par longueur de temps et par difficulté (il l'était trois fois par piège et par image) ; `params.slow` lu comme une fraction (`pl.gasSlowMul`, l'encens ralentit bien de 35 %) ; `lifetime` lu par la bouche de feu ; les lignes de la grille calculées une fois ; le dégradé de la nappe pré-rendu par rayon et par couleur, plus jamais recalculé par image ; l'annonce des dalles et de la nappe dans `PAL.alert` (elles étaient en corail codé en dur et en `#9f6`), les bordures des dalles et la nappe dans la couleur du piège (les spores sont violettes, la poudre beige, l'encens gris) ; **`beats.every` des émetteurs enfin musical** — « every: 4 » dans une partition voulait dire quatre temps et était lu comme deux secondes : le gyrophare, la gatling, le derviche, le diffuseur et le brumisateur jouent maintenant en mesure. *Gardé* : le type de salle `TRAP`, que l'audit disait mort — c'est celui de la salle de l'atelier.
- **Le test `pieges.js`** (14 contrôles) : pour chaque mécanique, un piège posé à la main, le joueur placé au point frappé à l'instant de la fenêtre active perd des points de vie, le piège se recharge (pas deux coups en une demi-seconde), rien hors fenêtre ni coupé, et `dangerAt` au même point vaut au moins 0,5 un quart de seconde avant ; les trois tireurs lancent leurs balles marquées `trap` aux dégâts du piège, jamais deux fois le même coup, et le suivant vient ; le gaz ralentit de sa valeur ; la cadence musicale est calculée une fois et refaite si la difficulté change ; deux pièges identiques donnent les mêmes phases ; les dix mécaniques sont des triplets. Le test se rejoue tel quel sur l'ancien moteur : 11 sur 14, et les trois ratés sont exactement les trois changements voulus (les tables, la valeur du ralentissement, le cache de la cadence) — tout ce qui décrit un coup, une recharge, un danger ou une salve passe à l'identique : c'est la preuve que le comportement n'a pas bougé.
- **Le chemin sûr prouvé** (`salles.js`, septième contrôle) : dans chacune des 25 salles à pièges, tous les pièges armés en même temps, la carte du danger est calculée toutes les 0,2 s sur 24 s ; depuis chaque instant de départ (toutes les 0,4 s sur 12 s), un parcours en largeur dans l'espace et le temps — avancer d'une tuile ou attendre, jamais sur une tuile où `dangerAt` ≥ 0,8 — doit atteindre la porte, et le sas doit être sûr les deux premières secondes. 711 départs, aucune coupure. La première version (un chemin à *chaque instant*) échouait dans neuf salles : un balayage qui traverse la salle la coupe forcément en deux pendant qu'il passe, et une nappe posée en (3,6) couvre le sas quand elle s'ouvre ; c'est le parcours dans le temps qui dit la vraie chose, « on peut toujours attendre puis passer ». La portée d'un tireur (0,5-0,6) ne barre pas : ses balles se lisent et s'esquivent.

Tests : `pieges.js` (14/14), `salles.js` (7/7), `etape0.js`, `perf.js` (adapté aux tables), `bat.js`, `palette.js`, `spawncheck.js`, `cadence.js` passent ; batterie complète 52/53, le raté étant `fin.js` (l'écran de fin, sans rapport avec les pièges), qui passe seul.

## 73. Chantier 13, séance B — les deux camps

**Un piège de WAY blesse maintenant tout ce qu'il touche : le joueur, les ennemis (plus fort), les boss (un peu), les compagnons (à moitié). Ses balles aussi. Les huit pièges qui dormaient dans les définitions sont posés. C'est le basculement du chantier : un piège n'est plus un impôt, c'est un outil.**

- **Les multiplicateurs** (`BALANCE.trap`) : `enemyMul` 1,5 (la règle d'Hades : les pièges blessent plus les ennemis que le héros), `bossMul` 0,25, `petMul` 0,5 ; 0 épargne un camp. Le joueur garde ses dégâts d'avant, à l'unité.
- **Le moteur** (`Trap.others`, `Trap.hitOther`, 34_traps.js) : à chaque image de fenêtre active, après le joueur, le corps du piège est testé contre chaque ennemi vivant et chaque compagnon présent (jamais le compagnon parti ni sonné) ; la même géométrie que pour le joueur (le rayon du corps de la cible en marge). Une recharge d'une demi-seconde **par cible** (une `Map`, sur l'horloge propre du piège : deux ennemis dans le même rayon sont frappés chacun). Un ennemi passe par `Combat.hitEnemy` en `dot` (ni critique, ni recul, ni point faible) ; un compagnon par `Pets.hurt` (il est sonné, jamais tué). Le nuage tique par sa recharge (une demi-dose toutes les 0,5 s), sans ralentir les ennemis (les statuts viennent à la séance C). **Les balles de piège** (`p.trap`, 30_entities.js) abattent l'ennemi ou le boss qu'elles croisent, à la même fraction, et disparaissent : une tourelle fauche ce qui passe devant elle, une couronne d'émetteur fauche tout autour.
- **Les huit pièges dormants posés** : le balayage dans le sous-sol de l'Admission (salle 4, un rayon vertical qui va et vient sur les flaques où l'on ralentit), deux rayons muraux dans le couloir des brancards (salle 8, l'un de la gauche sur la ligne 9, l'autre de la droite sur la ligne 3, en alternance), deux nappes de gaz dans la revanche de l'Admission (salle 9, de part et d'autre de l'arène) ; le treillis sur le pont de la Serre (salle 8, quatre lignes tous les quatre tuiles), deux lampes de culture dans la salle des bras d'agitation (salle 4, les couloirs du haut et du bas grillés en alternance), la tondeuse dans la revanche de la Serre (salle 9, sur la ligne 10) ; deux wagonnets fous dans le train de la Concession (salle 7, la ligne du haut et celle du bas) ; le moucharabieh dans les terrasses du Sérail (salle 4, deux zones de part et d'autre des claustras, le soleil passe). 41 pièges définis, 41 posés. `spawncheck.js` et le chemin sûr prouvé de `salles.js` passent avec les nouvelles poses (les tireurs muraux à x = 23 sont hors du couloir de la porte).
- **Le test** (`pieges.js`, 19 contrôles) : un ennemi dans le rayon prend ×1,5, une recharge par cible puis un second coup ; un boss ×0,25 ; un compagnon ×0,5 par `Pets.hurt` ; le nuage blesse un ennemi par tic ; une balle de tourelle abat l'ennemi qu'elle croise et disparaît.
- **Le banc** (`dev/test/bench/normal.js`, 8 armes × 4 graines, Admission, Martin et Uno, avant et après) : 32 parties de bot avant et après, mêmes graines. Victoires **8 → 8** (le critère « pas plus de 10 % » est tenu à l'unité) ; le bot atteint le boss **15 → 19** fois sur 32, la salle médiane passe de **5 à 6**, la durée médiane de 175 à 241 s, les ennemis abattus médians de 98 à 133, les dégâts subis médians de 197 à 225 (des parties plus longues), Uno sonné 1,09 → 0,97 fois par partie. Lecture : les pièges qui touchent les deux camps font avancer le bot d'une salle sans changer l'issue — un ennemi fauché par une tourelle est un ennemi de moins à tuer, et ce que le bot perd dans les nappes il le regagne. Les multiplicateurs restent donc ceux du plan (1,5 / 0,25 / 0,5), à l'auteur de dire à la main si un piège lui a servi.

Tests : `pieges.js` (19/19), `spawncheck.js`, `salles.js` (chemin sûr prouvé avec les nouvelles poses), `check-terrain` ; batterie complète 53/53.

## 74. Chantier 13, séance C — le joueur décide

**Les pièges répondent maintenant aux deux questions qui manquaient : à qui il sert, et comment je l'éteins. Des pièges qui se déclenchent à la plaque, à l'approche, au tir du joueur ou en chaîne ; qui poussent, endorment, retiennent, brûlent ou explosent ; qu'on casse ou qu'on désamorce d'un tir sur leur boîtier. Seize pièges nouveaux, quatre par palier, avec des sprites libres.**

- **Les déclencheurs à état** (`TRAP_TRIGGERS`, 34_traps.js) : `press` (une plaque : le joueur ou un ennemi se tient sur la zone posée, `who` any / player / enemy, ou sur le corps du piège si `sensor: 'body'`), `near` (une cible à moins de `near` tuiles du centre), `bullet` (armé par un tir du joueur ou par un piège lié), `ignite` (une balle de piège enflammée passe dessus). Ils dorment (`firedAt` nul), quelque chose les arme (`Trap.arm`), ils jouent une annonce puis une fenêtre (`armedStage`, pur : le bot regarde en avance), puis se réarment après `rearm` secondes — jamais si `once`. Une plaque endormie n'est pas un danger, mais le bot la voit à 0,3 : il n'a rien à y faire.
- **Les effets nouveaux** (`TRAP_EFFECTS`) : `push` (trois tuiles dans `pushAngle` ou loin du centre, sans dégât, une fois par demi-seconde et par cible, `kvx/kvy` du joueur et des ennemis), `status` (sur un ennemi `stun` et `slow`, sur le joueur `slowPlayer` et `rootPlayer` par `jamUntil` ; `bite` : des dégâts en plus ; un compagnon n'a pas de prise), `burn` (une zone au sol pendant `burnTime` s dans `room.hazards`, propriétaire `trap` : Room.update brûle le joueur et les ennemis à la fraction des deux camps), `blast` (une explosion pour les deux camps, en chaîne avec les pièges du même `link` à `linkRange` tuiles après `linkDelay` s ; `once` la consomme), `arm` (arme les pièges du même `link`), `call` (des ennemis arrivent et une bourse tombe : le trésor gardé). Une définition peut porter plusieurs effets (`effect: ['damage', 'status']`).
- **Le boîtier** : une définition (ou une pose) avec `hp` a un boîtier — au centre du piège, ou sur la tuile `box` ; une balle du joueur qui le touche (`Trap.shotBy`, `onShot`, branche joueur de `Projectiles.update`) enlève un point, et selon `onShot` : `break` (à zéro, désamorcé `rearm` secondes, 8 par défaut, voyant gris — on paie des tirs pour du calme), `fire` (le piège part maintenant : la bonbonne, les gousses, la vanne, le brancard), `flip` (la grille inverse ses lignes : le néon à bascule), `toggle` (le wagonnet change de voie : l'aiguillage, `routes`). Le voyant est orange pour un boîtier qui casse, de la couleur du piège pour un boîtier qui déclenche ; les coups qu'il reste sont dessinés dessous.
- **Le corps `prop`** : un accessoire dessiné en sprite (`params.sprite`, un PNG de `assets/sprites/pixel/`), une lueur d'alerte quand il est armé, une zone ronde ; consommé, il ne laisse qu'une ombre. Huit sprites découpés des packs CC0 (Kenney Industrial : bonbonne, gyrophare ; Kenney Pixel Platformer : plaques relevée et enfoncée, levier ; Kenney Tiny Dungeon : flaque ; Ninja Adventure : gousse ; Kenney Roguelike : poteau), `PROP_DEFS` en `png: true`.
- **Les seize pièges** (quatre par palier, `trapPool` et poses) — Admission : la bonbonne d'oxygène (deux en salle 3, un tir et elle saute pour tout le monde, la voisine avec elle), le défibrillateur (salle 3, une plaque, un arc qui blesse et étourdit), le brancard fou (salle 8, un tir et il fait un aller sur son rail, puis se range), le rideau de désinfection (salle 6, un nuage sans dégât où les ennemis rampent), et le néon à bascule (la grille du tempo gagne un boîtier derrière elle : un tir inverse les lignes). Serre : la dionée (salle 3, elle claque à une tuile, mord et retient), les gousses éclatantes (trois en salle 4, elles mûrissent en six secondes ou éclatent au tir), la vanne d'arrosage (salle 2, un tir et le jet pousse tout vers la droite sans blesser), le pollen soporifique (salle 7, les ennemis dorment deux secondes, toi tu ralentis). Concession : les tonneaux de poudre (quatre en file en salle 6, en chaîne), le fil de détente armé (salle 8, qui le franchit fait partir les dynamites liées), l'aiguillage (salle 7, le wagonnet du haut change de voie au tir sur le levier), la cloche du saloon (salle 4, une plaque qui appelle quatre bandits et lâche une bourse de 60). Sérail : les dalles du Vizir (salles 6, les pieux sortent sous qui marche, ennemis compris), la flaque d'huile (deux en salle 7, inertes jusqu'à la jarre de naphte, puis trois secondes de feu pour tout le monde), la cage à oiseaux (salle 2 et salle 8, elle tombe sur qui passe dessous). 57 pièges définis, tous posés.
- **L'atelier** : `KIND` connaît les onze familles nouvelles (taille, fenêtre, annonce, angle, longueur, sens) ; `TRAP_KINDS` est la liste des clés de `TRAP_LEGACY`, donc `Content.validate` accepte les familles nouvelles.
- **Le test** (`pieges.js`, 28 contrôles) : chaque mécanique du contenu est un triplet ; la plaque s'arme sous le joueur, l'arc blesse le joueur et l'ennemi et l'étourdit, puis se réarme après le délai ; la bonbonne saute au tir, blesse l'ennemi à côté, arme sa voisine avec un délai et se consomme ; deux tirs coupent une grille qui revient après le délai ; la vanne pousse sans blesser ; le pollen endort et ralentit sans blesser ; la flaque s'enflamme au passage d'une jarre et laisse un feu au sol ; la cage retient ; le brancard fait un aller puis se range. `spawncheck.js`, `vocabulaire.js` et le chemin sûr prouvé de `salles.js` passent avec les seize poses (une plaque endormie ne barre pas).

Tests : `pieges.js` (28/28), `spawncheck.js`, `salles.js`, `vocabulaire.js` ; batterie complète 53/53.

## 75. Chantier 13, séance D — ce qui bouge et ce qui reste, et le bilan du chantier

**Le piège porté passe par les tables, un piège peut suivre un mur coulissant, le sablier de salle décale toutes les phases d'un tir, la cloche garde un coffre, les dalles et les bouches de feu ont leurs sprites, et le chantier 13 est fini : les pièges de WAY répondent aux trois questions.**

- **Le piège porté** : `Room.dropTrap(id, x, y)` pose un vrai piège en cours de salle, à usage unique, hors des familles du tempo ; `behavior.drop: 'trap_loup'` sur le coyote remplace le `hazard` codé à la main dans `Combat.killEnemy` (la branche morte de `Room.update` est retirée). `trap_loup` : une plaque pour le joueur seul, une annonce de 0,15 s, une morsure, puis il disparaît — un piège `once` se consomme une fois sa fenêtre passée.
- **Le parent** (`params.parent = { modular, dx, dy }`) : le piège suit le premier obstacle de l'élément modulaire de la salle, décalé de (dx, dy) tuiles, une image de retard invisible. Posé : des pointes sur le premier mur coulissant de la salle 6 de l'Admission.
- **Le sablier de salle** (`trap_sablier`, tous paliers, kind `hourglass` : tir + accessoire + effet `shift`) : un tir le retourne et `room.trapShift` avance de 1,5 s — `Room.trapTime` l'ajoute aux pièges à l'horloge de salle, jamais à ceux en musique. Posé dans la salle 3 de l'Admission, la 6 de la Serre, la 3 de la Concession, la 4 du Sérail. On choisit la phase de la salle au lieu de la subir.
- **Le trésor gardé** : la cloche du saloon ouvre aussi un coffre (`params.chest` → `Room.offerChest`) : quatre bandits contre une greffe et une bourse.
- **Les sprites** : la gargouille de Kenney Tiny Dungeon (sèche au repos, qui crache pendant l'annonce, teintée à la couleur du piège) pour les quatre bouches de feu ; les quatre images des dalles de 0x72 (nue, qui perce, sortie, sortie) pour les dalles, épines, pièges à ours, pieux et dalles du Vizir, avec le liseré d'alerte par-dessus pendant l'annonce ; la cage dessinée en barreaux, suspendue au repos et au sol quand elle tombe. `Sprites.hasProp` dit si une planche est là ; sans elle, le vecteur d'avant.
- **Les tests** : `pieges.js` à 32 contrôles (le coyote tombe et un piège à loup apparaît, mord une fois, disparaît ; le parent suit ; le sablier décale la salle et pas la musique ; la cloche appelle, paie et ouvre) ; `comportements.js` lit le piège à loup dans `room.traps` ; le chemin sûr prouvé et `spawncheck.js` passent avec les poses nouvelles (le sablier de la Concession a dû quitter la roche). `check-terrain.js` et `salles.js` n'ont pas eu besoin d'autre chose : un piège porté par un module n'est pas un obstacle, et sa position de repos est celle qui compte pour le chemin sûr.

**Le bilan du chantier 13** — quatre séances le même jour. Avant : dix mécaniques, 41 pièges, tout au temps, seul le joueur blessé, rien de cassable, huit pièges jamais posés, aucun test de dégât. Après : trois tables (déclencheur × corps × effets), 22 familles, 59 pièges définis et tous posés, les deux camps touchés, quatre déclencheurs à état, huit effets, le boîtier, le piège porté, le parent, le sablier ; `pieges.js` (32) et le chemin sûr prouvé dans le temps et l'espace ; le banc inchangé en victoires et le bot une salle plus loin. Ce que l'auteur doit dire : si un piège lui a servi.

Tests : `pieges.js` (32/32), `comportements.js`, `spawncheck.js`, `salles.js`, `vocabulaire.js` ; batterie complète 52/53, le raté étant `animaux.js` sous la charge (passe seul) ; `levels.js` sur les quatre paliers, trois armes chacun : le bot va au bout sans plantage partout, gagne trois fois sur trois à l'Admission, meurt ailleurs comme d'habitude, et tourne en rond deux fois en salle 2 de la Concession (déjà vu avant le chantier : la salle à défi) et une fois en salle 3 de la Serre avec le boomerang — à surveiller, c'est la salle des dionées.

## 76. Finitions après le chantier 13 — la lisibilité dans l'action

**Quatre remarques de l'auteur après une partie (« les ennemis et alliés sont difficiles à voir dans l'action, le pointeur devrait être plus visible, il faut un délai avant de pouvoir cliquer après une montée de niveau, les bruitages des armes sont trop forts pour un jeu où l'on reste cliqué, et il y a peut-être trop d'éléments à l'écran par moments »), un agent sur la surcharge, et une passe de lisibilité.**

- **Voir les ennemis et les alliés** : un contour sombre de 2 px autour de chaque sprite (`Sprites.outline`, `silhouetteFrame` / `drawOutline` dans 15_sprites.js — la silhouette noire d'une image, mise en cache comme le flash, dessinée quatre fois autour ; joueur, compagnons, ennemis et ennemis-accessoires) ; un anneau au sol sous chaque ennemi (corail `PAL.danger`, les boss plus marqués) et sous chaque compagnon (vert `PAL.life`), dessinés avec l'ombre. Un ennemi se repère à son anneau avant même son dessin.
- **Le réticule** (`renderReticle`, 50_ui.js) : le curseur du système est caché sur le jeu (`canvas.style.cursor = 'none'`) et remplacé par un anneau cyan `PAL.self` cerclé de noir, quatre crans, un point ; il revient sur tout écran, au tactile, en pause, mort.
- **Le verrou de l'écran de choix** (`UI.CHOICE_LOCK_MS`, 600 ms) : on arrive sur cet écran en plein tir, le doigt encore sur le bouton ; un clic ou une touche avant le délai ne choisit rien, les cartes sont voilées pendant ce temps (`.choice.cooling`).
- **Les armes plus discrètes** (`AudioEngine.LEVELS`) : les six tirs baissés de 40 % (lame 1,7 → 1, arc 1,3 → 0,8, boomerang 1,7 → 1, chaîne 2,2 → 1,3, pistolet 0,5 → 0,3, marteau 0,7 → 0,45 ; orbe et flamme à 0,7), et une rafale de tirs s'adoucit à 0,8 par coup rapproché au lieu de 0,88 — un tir maintenu tombe au quart de son premier coup.
- **La surcharge de l'écran** (rapport de l'agent, gardé dans `dev/agents/ecran.md` : quinze captures, le compte de tout ce qui se dessine par image, une hiérarchie en quatre niveaux et un budget d'écran mesurable). Le constat : le coût n'est pas le problème (60 i/s, un à deux flous), c'est le nombre de choses qui parlent — le décor au sol faisait la taille d'un ennemi (43 px contre 29 à 72), quatre ennemis verts sur sept en Serre sur un sol vert, des points lumineux de 4 à 8 px indistincts (balles ennemies, orbes, particules), tout qui bat dans la salle du tempo, des chiffres et des toasts empilés. Appliqué tout de suite, ce qui est sûr : le décor au sol à 0,7 tuile (1,05 en grand) et 60 % ; les balles ennemies cerclées de noir puis de corail ; le halo des tirs du joueur 12 → 6, celui des orbes d'XP 8 → 4, des pièces 8 → 5 ; le point du tempo au-dessus des ennemis éteint entre deux temps ; le bandeau « Vague n » hors du rouge (réservé au danger) ; les toasts « sonné / de retour » du compagnon retirés (le badge le dit) ; les chiffres de dégâts 18 → 14 px et 0,7 → 0,5 s, les critiques 30 → 24 ; l'égaliseur et le voile de la salle du tempo deux fois plus discrets. Gardé pour plus tard, parce que touché par des tests de ressenti ou à discuter : la teinte du décor (−30 % de saturation, jamais rouge, vert vie, or ni cyan), les particules (7 → 5 par coup, 36 → 22 par mort), la fusion des chiffres (un par ennemi par 0,3 s), les lumières animées à 60 % en combat, le trait néon sous les obstacles.
- **Les tests** : `finitions.js` (le contour : un ennemi sur un sol clair a bien plus de pixels sombres avec que sans ; l'anneau ; le réticule dessiné et le curseur caché puis revenu ; le verrou : un clic immédiat ne choisit rien, un clic après 600 ms choisit ; les niveaux de tir et la rafale) et `ecran.js` (le budget : en combat jamais plus de 250 particules, 8 chiffres ni 2 toasts — mesuré 31 à 37 particules, 1 à 2 chiffres ; et chaque réglage ci-dessus lu dans le code). `coup.js` suit les nouvelles tailles.

Tests : `finitions.js` (5/5), `ecran.js` (10/10), `coup.js`, `ressenti.js`, `bat.js`, `hud.js`, `lueurs.js`, `etape0.js`, `palette.js`, `interface.js`, `perf.js` ; batterie complète 55/55 (après deux corrections trouvées par elle : pas de contour pendant le flash blanc de la mort, `butin.js` ; les silhouettes dans leur propre cache, pas celui des flashs que `cadence.js` mesure).
