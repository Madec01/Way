...
function test(fn, opts = {}) {
  ...
  (async () => {
    ...
    let total = 0,
      echecs = 0;
    const ok = (libelle, cond, detail) => {
      total++;
      if (!cond) echecs++;
      console.log(`${cond ? '✓' : '✗ ÉCHEC'}  ${libelle}${detail ? ' — ' + detail : ''}`);
      return !!cond;
    };
    ...
    const salle = async (n, biome = 'biome_1') => {
      await page.evaluate(
        ([n, b]) => {
          const s = document.querySelector('#d-biome');
          if (s) s.value = b;
          Debug.gotoRoom(n);
          Debug.hide();
          G.debug.invuln = true;
    ...
    const run = async ({ character, pet, petMode = 'always' } = {}) => {
      await page.evaluate(
        ([c, p, m]) => {
          if (c) Meta.profile.character = c;
          if (p !== undefined) Meta.profile.pet = p;
          Meta.profile.petMode = m;
          Meta.save();
          document.getElementById('hub-enter').click();
        },
        [character, pet, petMode]
      );
      await page.waitForTimeout(1500);
      if (await page.$('[data-s]')) {
    ...
    let plante = null;
    try {
      await fn({ page, context, browser, ok, entrer, salle, run, sansPause, out, url, erreurs });
    } catch (e) {
      plante = e;
    }
    await browser.close();
    ...
    console.log(
      `${nom} : ${total - echecs}/${total} OK${echecs ? ` · ${echecs} ÉCHEC(S)` : ''}${errJs.length ? ` · ${errJs.length} ERREUR(S) JS` : ''}`
    );
    if (errJs.length) console.log('ERREURS JS\n  ' + errJs.slice(0, 5).join('\n  '));
    if (plante) {
      console.log('LE TEST A PLANTÉ : ' + (plante.stack || plante.message));
      process.exit(2);
    }
