...
test(async ({ page: p, context, ok, entrer, salle, run, sansPause, erreurs: errs }) => {
  ...
  await p.setInputFiles('#atelier [data-file="0"]', out('chat.png'));
  ...
  await p.setInputFiles('#atelier [data-filec="0"]', out('visage.png'));
  await p.waitForTimeout(700);
  await p.evaluate(() => {
  ...
  ok(
    ...
    Math.round(snip.length / 1024) + ' Ko pour deux amis'
  );
  ...
  const rechargé = await p.evaluate(txt => {
    ...
    return {
      ...
      nomPet: CONTENT2.pets[0] && CONTENT2.pets[0].name,
      nomChar: CONTENT2.characters[0] && CONTENT2.characters[0].name,
    };
  }, snip);
  ok(
    'le fichier exporté se relit sans erreur',
    rechargé.pets === 1 && rechargé.chars === 1 && rechargé.imgs === 2,
