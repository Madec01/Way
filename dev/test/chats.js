...
test(async ({ page: p, context, ok, entrer, salle, run, sansPause, erreurs: errs }) => {
  ...
  const defs = await p.evaluate(() =>
    Content.pets().map(x => ({ id: x.id, nom: x.name, clips: Object.keys(x.anim || {}).length, taille: x.size, place: !!x.placeholder }))
  );
  ok(
    'les quatre animaux ont leurs planches',
    defs.every(x => x.clips === 4 && !x.place),
    defs.map(x => `${x.nom} ${x.clips} clips, taille ${x.taille}`).join(' · ')
  );

  const sheets = await p.evaluate(() => {
    const o = {};
    for (const q of Content.pets())
      for (const [c, n] of Object.entries(q.anim || {})) {
        const s2 = Sprites.sheetInfo(n);
        o[n] = s2 ? `${s2.fw}px ×${s2.n}` : 'ABSENTE';
      }
    return o;
  });
  ok(
    'toutes les planches sont lues',
    Object.values(sheets).every(v => v !== 'ABSENTE'),
  ...
  const mes = await p.evaluate(() => {
    ...
    const boite = dessin => {
      ...
      dessin(g);
      const d = g.getImageData(0, 0, 220, 260).data;
      let t = 999,
        bt = -1,
        l = 999,
        r = -1;
    ...
    for (const d of Content.pets()) {
      ...
      q.dx = 1;
      q.dy = 0;
      const m = boite(g => q.render(g));
      if (q.airborne) m.bas += 15; // un animal qui vole plane 15 px au-dessus du sol : on le ramène
      out[d.name] = m;
    }
    const ch = Content.character('char_martin');
    out.Martin = boite(g =>
      Sprites.drawBody(g, ch.sprite, 110, 160, { anim: ch.anim, size: ch.size, clip: 'idle', clipT: 0, dir: 'e', walk: 0 })
    );
    return out;
  });
  ...
  ok(
    'les quatre posent les pattes sur la ligne de sol du joueur',
    Math.max(...bas) - Math.min(...bas) <= 2,
    `de ${Math.min(...bas)} à ${Math.max(...bas)}`
  );
  ...
  ok(
    'chaque animal est affiché à ×2 de sa case',
    grain.every(x => x.k === 2),
    grain.map(x => `${x.nom} ×${x.k}`).join(' · ')
  );

  /* en jeu, avec leurs maîtres */
  for (const [ch, pet, attendu] of [
    ['char_gabriel', 'pet_choupi', 'La maisonnée'],
    ['char_jean', 'pet_ori', 'Œil pour œil'],
  ]) {
    const r = await p.evaluate(
      async ([c, q]) => {
        if (G.run) Run.toHub();
        await new Promise(x => setTimeout(x, 500));
        Meta.profile.character = c;
        Meta.profile.pet = q;
    ...
    ok(
      `${r.perso} entre avec ${r.animaux.map(x => x.split(':')[0]).join(' & ')}`,
      r.equipe === attendu,
      r.equipe + ' · clips ' + r.animaux.join(' ')
    );
  }
  ...
  const clip = await p.evaluate(() => {
    const q = G.pets[0];
    const o = {};
    q.animStep(0.016, false);
    o.arret = q.clip;
    for (let i = 0; i < 3; i++) q.animStep(0.016, true);
    o.marche = q.clip;
    q.act = 1;
    q.animStep(0.016, false);
    o.action = q.clip;
  ...
  ok(
    "leurs clips suivent ce qu'ils font",
  ...
  const vign = await p.evaluate(() => {
    const out = {};
    for (const q of Content.pets()) {
      const c = Sprites.sheetCanvas(q.anim.idle, 32);
      out[q.name] = !!c;
    }
    return out;
  });
