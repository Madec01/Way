...
test(async ({ page, ok, url }) => {
  ...
  for (let i = 0; i < armes.length; i++) {
    const cfg = {
      ...
      maxSeconds: 700,
      difficulty: 1,
      biome: process.env.BIOME || null,
    };
    const r = await Promise.race([
      page.evaluate(c => window.__autoplay(c), cfg),
      new Promise(res => setTimeout(() => res({ outcome: 'HARNESS_TIMEOUT' }), 120000)),
    ]);
    ...
    ok(
      `${armes[i]} : le bot va au bout sans plantage`,
      r.outcome === 'victory' || r.outcome === 'death' || r.outcome === 'timeout',
      detail
    );
