...
test(async ({ page: p, context, ok, entrer, salle, run, sansPause, erreurs: errs }) => {
  ...
  ok(
    'les trois personnages sont là',
    roster.chars.includes('Martin') && roster.chars.includes('Gabriel') && roster.chars.includes('Jean'),
    roster.chars.join(' · ')
  );
  ok('les quatre animaux sont là', roster.pets.length === 4, roster.pets.join(' · '));
  ok(
    "la boutique n'offre pas Tanuki tout seul",
    roster.boutique.length === 3 && roster.boutique.includes('Choupi & Tanuki') && !roster.boutique.includes('Tanuki'),
    roster.boutique.join(' | ')
  );
  ...
  const duo = await p.evaluate(async () => {
    ...
    return {
      n: G.pets.length,
      noms: G.pets.map(x => x.name),
      comportements: G.pets.map(x => x.def.behavior),
      meneur: G.pet && G.pet.name,
      muls: G.pets.map(x => x.pairMul),
    };
  });
  ok(
    'choisir Choupi amène aussi Tanuki',
    duo.n === 2 && duo.noms.join(',') === 'Choupi,Tanuki',
    duo.noms.join(' + ') + ' — ' + duo.comportements.join(' + ')
  );
  ok(
    "le bonus d'équipe vaut pour les deux",
    duo.muls.every(m => m === 1.3),
    'chacun ×' + duo.muls[0]
  );
  ...
  const vie = await p.evaluate(async () => {
    ...
    const e = Room.spawnEnemy(def, G.player.x + 260, G.player.y, {});
    e.hp = 99999;
    e.speed = 0;
    ...
    G.player.y += 90;
    Pickups.list = [];
    for (let i = 0; i < 4; i++) Pickups.spawn(G.player.x + 120 + i * 10, G.player.y + 60, 'coin', 1);
    let roula = false;
    const id = setInterval(() => {
      if (G.pets.some(x => x.state === 'roll')) roula = true;
    }, 30);
    await new Promise(r => setTimeout(r, 6000));
    clearInterval(id);
    const pres = G.pets.map(x => Math.round(Math.hypot(x.x - G.player.x, x.y - G.player.y)));
    return {
      bougé: G.pets.filter((x, i) => Math.hypot(x.x - p0[i].x, x.y - p0[i].y) > 20).length,
      pres,
      roula,
      aimantés: Pickups.list.filter(x => x.magnet).length,
      restants: Pickups.list.length,
      degats: Math.round(99999 - e.hp),
    };
  });
  ...
  ok(
    'Choupi aimante ce qui traîne',
    vie.aimantés > 0 || vie.restants === 0,
    vie.restants === 0 ? 'les 4 pièces sont déjà ramassées' : vie.aimantés + ' ramassables attirés'
  );
  ...
  const ori = await p.evaluate(async () => {
    ...
    G.enemies = [];
    const def = Content.enemy(Content.biome('biome_1').enemyPool[0]);
    const e = Room.spawnEnemy(def, G.player.x + 200, G.player.y, {});
    e.hp = 99999;
    e.speed = 0;
    await new Promise(r => setTimeout(r, 2600));
    const marque = e.markUntil > Time.now;
    const hp0 = e.hp;
    Combat.hitEnemy(e, 100, { noCrit: true, silent: true });
    const avec = hp0 - e.hp;
    e.markUntil = 0;
    const hp1 = e.hp;
    Combat.hitEnemy(e, 100, { noCrit: true, silent: true });
  ...
  await p.screenshot({ path: out('duo.png') });
});
