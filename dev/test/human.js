...
test(async ({ page, ok, entrer }) => {
  await entrer('normal');
  const hub = await page.evaluate(() => ({ coins: Meta.coins, weapons: Meta.profile.weapons.length, chars: Meta.profile.characters.length, mode: G.mode }));
  ok('le hub s\'ouvre en mode Normal', hub.mode === 'normal', `${hub.coins} crédits · ${hub.weapons} armes · ${hub.chars} personnages`);
  await page.click('#hub-enter'); await page.waitForTimeout(900);
  const prep = await page.$('[data-s]');
  if (!ok('l\'écran de préparation propose une compétence', !!prep)) return;
  await page.click('[data-s]'); await page.click('#prep-go'); await page.waitForTimeout(1200);
  ...
  await page.mouse.move(900, 360); await page.mouse.down(); await page.waitForTimeout(500); await page.mouse.up();
  ...
  await page.evaluate(() => { G.run.coinsPending = 100; G.run.coinsValidated = 50; G.player.hp = 1; Combat.hitPlayer(999, { type: 'contact' }); });
  await page.waitForTimeout(400);
  const fin = await page.evaluate(() => ({ overlay: G.overlay, coins: Meta.coins, deaths: Meta.profile.deaths }));
  ok('la mort garde 50 + 10 % de 100 = 60 crédits', fin.overlay === 'end' && fin.coins === 60, `${fin.coins} crédits, ${fin.deaths} mort(s)`);
  ...
  await entrer('normal');
  const achat = await page.evaluate(() => { const r = Meta.buy('meta_vitalite'); return { r, tier: Meta.tierOf('meta_vitalite'), coins: Meta.coins }; });
