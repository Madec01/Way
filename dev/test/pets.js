...
test(async ({ page: p, context, ok, entrer, salle, run, sansPause, erreurs: errs }) => {
  ...
  const cov = await p.evaluate(() => ({
    manque: PET_BEHAVIORS.filter(b => !Content.pets().some(p2 => p2.behavior === b)),
    total: PET_BEHAVIORS.length,
  }));
  ok(
    'chaque comportement déclaré est couvert',
    cov.manque.length === 0,
    cov.manque.length ? 'non couverts : ' + cov.manque.join(', ') : cov.total + ' comportements'
  );

  await p.evaluate(() => {
    document.querySelector('#d-biome').value = 'biome_1';
    Debug.gotoRoom(2);
    Debug.hide();
    G.debug.invuln = true;
  ...
  const tortue = await p.evaluate(async () => {
    Pets.give('pet_tortue');
    G.enemies = [];
    Projectiles.list = [];
    let brises = 0;
    const n0 = 40;
    for (let i = 0; i < n0; i++) {
      const a = (i * Math.PI * 2) / n0;
      Projectiles.spawn({
        x: G.player.x + Math.cos(a) * 90,
        y: G.player.y + Math.sin(a) * 90,
        vx: -Math.cos(a) * 120,
        vy: -Math.sin(a) * 120,
        r: 5,
        damage: 1,
  ...
  const scara = await p.evaluate(async () => {
    ...
    Pets.give('pet_scarabee');
    G.pet.x = G.player.x + 200;
    G.pet.y = G.player.y + 110;
  ...
  ok(
    'le scarabée aimante ce qui traîne',
    scara.sans === 0 && scara.avec > 0,
    `sans lui ${scara.sans} aimantée(s), avec lui ${scara.avec}/6`
  );
  ...
  const chien = await p.evaluate(async () => {
    ...
    G.pet.y = 360;
    const def = Content.enemy(Content.biome('biome_1').enemyPool[0]);
    const e = Room.spawnEnemy(def, 700, 360, {});
    e.hp = 9999;
    await new Promise(r => setTimeout(r, 400));
  ...
  const serpent = await p.evaluate(async () => {
    Pets.give('pet_serpent');
    Projectiles.list = [];
    G.enemies = [];
    G.player.x = 400;
    G.player.y = 360;
    const def = Content.enemy(Content.biome('biome_1').enemyPool[0]);
    const e = Room.spawnEnemy(def, 560, 360, {});
    e.hp = 9999;
    e.speed = 0;
    let n = 0;
    const s = Projectiles.spawn.bind(Projectiles);
    Projectiles.spawn = o => {
      if (o.owner === 'player') n++;
      return s(o);
    };
    await new Promise(r => setTimeout(r, 6000));
    Projectiles.spawn = s; // il crache toutes les 2 mesures : 3 s peuvent ne rien contenir
    return { n };
  });
  ...
  const drop = await p.evaluate(async () => {
    ...
    const avant = !!G.pet;
    G.player.x = Pickups.list[0].x;
    G.player.y = Pickups.list[0].y;
