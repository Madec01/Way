...
test(async ({ page, ok, entrer }) => {
  ...
  const t1 = await page.evaluate(() => {
    ...
    return {
      existe: !!rot,
      aBeats: !!(rot && rot.beats),
      musical: rot ? Room.trapTime(G.room, rot) === Beat.t : null,
      autre: (() => {
        const o = G.room.traps.find(t => !t.beats);
        return o ? Room.trapTime(G.room, o) === G.room.time : 'aucun';
      })(),
      tempo: !!G.room.tempo,
    };
  });
  ...
  const t2 = await page.evaluate(() => {
    ...
    const tourEnTemps = (Math.PI * 2) / rot.p.angularSpeed / L;
    return { angularSpeed: +rot.p.angularSpeed.toFixed(4), tourEnTemps: +tourEnTemps.toFixed(3), L: +L.toFixed(3) };
  });
  ok(
    'un tour dure exactement 4 temps',
    Math.abs(t2.tourEnTemps - 4) < 0.01,
    `${t2.tourEnTemps} temps · ω=${t2.angularSpeed} rad/s · 1 temps=${t2.L}s`
  );
