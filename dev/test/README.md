...
| `cadence` | chantier 9 : quatre salles cadencées du biome 1 (partition, pouls, dalles dessinées), wagonnets du train, masque des lumières une image sur deux et sous 2 ms, cache du flash |
...
| `dette` | chantier 11 : les tables BOSS_PATTERNS / PET_ACTIONS / UPDATERS couvrent tout le contenu et remplacent les switch, `Content.character` / `biome` inconnus → null + console, les nombres de la difficulté dans BALANCE, plus aucun reste « SALLE ZÉRO » / TODO_SPRITE / deepClone dans dev/ |
| `sols` | chantier 12 : un sol et des murs par palier (planches CC0 chargées, tileset par palier, couleurs distinctes, motifs du mur du haut, fontaine animée, sol identique à chaque visite, repli 0x72, pas de flou de plus) |
| `reprise` | retours après le chantier 10 : le zoom de scène (mort 1,3 / victoire 1,15) remis à 1 à l'entrée de salle, le cadrage qui tient compte du zoom de scène (la porte de droite visible), la musique qui repart quand on relance pendant la mort de la bande |
| `paysage` | chantier I-9 : en 900×420 tactile, le plancher d'échelle des menus, le camp en deux colonnes et moins de 2,5 écrans, l'en-tête et PARTIR en une ligne, la phrase du palier sous la rangée, la prépa en rangées avec en-tête collant ; au bureau rien ne bouge |
| `perf` | chantier 10 : Halo.line/rect, plus de flou par image dans les pièges, modules, tempo, porte et lames (≤ 7 par image en salle), la mesure (i/s, minimum, rendu, flous) et sa ligne, le rendu économe qui coupe et se remet, l'automatique au tactile après 3 s lentes, la pause, le compteur, les boutons tactiles sans flou, `?perf` |
| `comparer` | chantier 7 : le score d'une partie, la graine reproductible et la graine du jour, l'écran de fin avec la ligne à copier, la ligne collée qui rejoue la graine, les dix meilleures parties, l'export et l'import de la sauvegarde |
...
| `pieges` | chantier 13 A : chaque mécanique de piège blesse au point et à l'instant attendus, se recharge, ne fait rien hors fenêtre ni coupée, prévient le bot un quart de seconde avant ; les tireurs lancent leurs balles marquées `trap` aux dégâts du piège sans rejouer un coup ; le ralentissement du gaz lit sa valeur ; la cadence musicale est calculée une fois ; deux pièges identiques ont les mêmes phases ; les dix mécaniques sont des triplets de tables (se rejoue tel quel sur l'ancien moteur) ; 13 B : un ennemi dans le piège prend ×1,5 avec une recharge par cible, un boss ×0,25, un compagnon ×0,5, le nuage par tic, une balle de tourelle abat l'ennemi croisé ; 13 C : la plaque s'arme et se réarme, la bonbonne saute au tir et en chaîne puis se consomme, deux tirs coupent une grille qui revient, la vanne pousse sans blesser, le pollen endort, la flaque s'enflamme, la cage retient, le brancard fait un aller ; 13 D : le coyote laisse un piège à loup qui mord une fois et disparaît, le parent suit le mur coulissant, le sablier décale la salle et pas la musique, la cloche appelle, paie et ouvre un coffre |
...
| `salles` | chantier 12 D et 13 A : les 36 salles créées sans être lancées — porte atteignable à pied et par un couloir de deux tuiles, sas d'entrée et couloir de la porte libres (obstacle, terrain solide, piège), carré 3×3 libre pour le boss, décor sur sol nu, ≤ 40 obstacles ; le chemin sûr prouvé (à tout instant un parcours sans danger de piège relie le sas à la porte) |
