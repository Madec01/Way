...
| `pieges` | chantier 13 A : chaque mécanique de piège blesse au point et à l'instant attendus, se recharge, ne fait rien hors fenêtre ni coupée, prévient le bot un quart de seconde avant ; les tireurs lancent leurs balles marquées `trap` aux dégâts du piège sans rejouer un coup ; le ralentissement du gaz lit sa valeur ; la cadence musicale est calculée une fois ; deux pièges identiques ont les mêmes phases ; les dix mécaniques sont des triplets de tables (se rejoue tel quel sur l'ancien moteur) ; 13 B : un ennemi dans le piège prend ×1,5 avec une recharge par cible, un boss ×0,25, un compagnon ×0,5, le nuage par tic, une balle de tourelle abat l'ennemi croisé |
