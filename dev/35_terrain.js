...
const TERRAIN_BY_ID = []; for (const ch in TERRAIN) { TERRAIN_BY_ID[TERRAIN[ch].id] = Object.assign({ ch }, TERRAIN[ch]); }
...
const Terrain = (() => {
  ...
  function codeAt(x, y) {
    const g = G.room && G.room.grid; if (!g) return 0;
    const [tx, ty] = tileOf(x, y); if (!inside(tx, ty)) return 0;
    return g[idx(tx, ty)];
  }
  const at = (x, y) => defOf(codeAt(x, y));
  /* on peut se tenir ici : ni terrain solide, ni obstacle déclaré */
  function walkable(tx, ty, room) {
    const r = room || G.room; if (!r) return false;
    if (!inside(tx, ty)) return false;
    if (r.grid && defOf(r.grid[idx(tx, ty)]).solid) return false;
    const px = ROOM_X + (tx + 0.5) * TILE, py = ROOM_Y + (ty + 0.5) * TILE;
    for (const o of r.obstacles) { if (o.terrain) continue; if (circleRect(px, py, TILE * 0.35, o.px, o.py, o.pw, o.ph)) return false; }
    return true;
  }
  /* tuile praticable la plus proche d'un point (replacement après une poussée dans un mur) */
  function nearestWalkable(x, y, room) {
    const r = room || G.room; const [cx, cy] = tileOf(x, y);
    if (walkable(cx, cy, r)) return { x, y };
    for (let ring = 1; ring < 12; ring++) {
      let best = null, bd = Infinity;
      for (let ty = cy - ring; ty <= cy + ring; ty++) for (let tx = cx - ring; tx <= cx + ring; tx++) {
        if (Math.max(Math.abs(tx - cx), Math.abs(ty - cy)) !== ring || !walkable(tx, ty, r)) continue;
        const px = ROOM_X + (tx + 0.5) * TILE, py = ROOM_Y + (ty + 0.5) * TILE; const d = dist(x, y, px, py);
  ...
  function plain(tx, ty, room) { const r = room || G.room; return !r.grid || (inside(tx, ty) && r.grid[idx(tx, ty)] === 0); }
  /* multiplicateur de vitesse de la surface (1 hors terrain ralentissant) */
  function speedAt(x, y) { const t = at(x, y); return t.speed || 1; }
  /* le point coupe-t-il la vue sans bloquer les pas (moucharabieh, feuillage, vapeur) */
  function screenAt(x, y) { const t = at(x, y); return !t.solid && !!t.blocksSight; }
  ...
  function compile(room, def) {
    const plan = def.terrain, rects = def.terrainRects;
    ...
    if (plan) {
      for (let ty = 0; ty < Math.min(plan.length, ROOM_ROWS); ty++) {
        ...
        for (let tx = 0; tx < Math.min(line.length, ROOM_COLS); tx++) {
          ...
          if (!t) { console.warn('[Terrain] caractère inconnu « ' + line[tx] + ' » en ' + tx + ',' + ty + ' (' + def.id + ')'); continue; }
          g[idx(tx, ty)] = t.id;
        }
      }
      if (plan.length !== ROOM_ROWS) console.warn('[Terrain] ' + def.id + ' : ' + plan.length + ' lignes au lieu de ' + ROOM_ROWS);
      for (const line of plan) if (line.length !== ROOM_COLS) { console.warn('[Terrain] ' + def.id + ' : une ligne fait ' + line.length + ' caractères au lieu de ' + ROOM_COLS); break; }
    }
    for (const rc of (rects || [])) {
      const t = TERRAIN[rc.ch] || Object.values(TERRAIN).find(v => v.name === rc.kind);
      if (!t) { console.warn('[Terrain] rectangle de type inconnu', rc); continue; }
      for (let ty = rc.y; ty < rc.y + (rc.h || 1); ty++) for (let tx = rc.x; tx < rc.x + (rc.w || 1); tx++) if (inside(tx, ty)) g[idx(tx, ty)] = t.id;
    }
    ...
    for (let ty = 0; ty < ROOM_ROWS; ty++) {
      ...
      for (let tx = 0; tx <= ROOM_COLS; tx++) {
        ...
        if (solid && c === code) { run++; continue; }
        if (run) pushRect(room, tx - run, ty, run, 1, code);
        run = solid ? 1 : 0; code = solid ? c : 0;
      }
    }
  ...
  function pushRect(room, tx, ty, w, h, code) {
    const t = defOf(code);
    room.obstacles.push({ x: tx, y: ty, w, h, kind: t.name, terrain: true, code,
      stopsShot: t.stopsShot !== false, blocksSight: t.blocksSight !== false, dashOver: !!t.dashOver,
      px: ROOM_X + tx * TILE, py: ROOM_Y + ty * TILE, pw: w * TILE, ph: h * TILE });
  }
  /* parcours en largeur de l'entrée vers la porte : une salle coupée en deux est une run perdue */
  function connected(room) {
    const seen = new Uint8Array(ROOM_COLS * ROOM_ROWS); const q = [[1, Math.floor(ROOM_ROWS / 2)]];
    const start = q[0]; if (!walkable(start[0], start[1], room)) return false;
  ...
  function paint(g, room) {
    ...
    for (let ty = 0; ty < ROOM_ROWS; ty++) for (let tx = 0; tx < ROOM_COLS; tx++) {
      const t = defOf(grid[idx(tx, ty)]); if (t.id === 0) continue;
      const s = skinOf(t.name); if (!s || !s.fill) continue;
      const x = ROOM_X + tx * TILE, y = ROOM_Y + ty * TILE;
      g.globalAlpha = s.alpha != null ? s.alpha : 1; g.fillStyle = s.fill; g.fillRect(x, y, TILE, TILE);
      /* liseré de rive : c'est lui qui rend la surface lisible d'un coup d'œil, pas le remplissage */
      g.globalAlpha = 1; g.fillStyle = s.edge;
      ...
      if (s.planks) { g.globalAlpha = 0.5; for (let k = 6; k < TILE; k += 12) g.fillRect(x + 2, y + k, TILE - 4, 2); g.globalAlpha = 1; }
      if (s.lattice) { g.globalAlpha = 0.45; for (let k = 6; k < TILE; k += 10) { g.fillRect(x + k, y, 2, TILE); g.fillRect(x, y + k, TILE, 2); } g.globalAlpha = 1; }
    }
    g.globalAlpha = 1;
  }
  ...
  function render(ctx, room) {
    if (!room.grid) return;
    const grid = room.grid; const t = Time.now;
    ...
    for (let ty = 0; ty < ROOM_ROWS; ty++) for (let tx = 0; tx < ROOM_COLS; tx++) {
      ...
      ctx.globalAlpha = 0.05 + 0.07 * k; ctx.fillStyle = '#9fd8ff';
