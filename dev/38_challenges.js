...
const Challenge = (() => {
  ...
  function create(id, room) {
    ...
    if (id === 'capture') {
      ...
      c.beams = []; c.patId = null; c.patName = ''; c.patBars = 4; c.barAt = 0; c.fade = 1; c.flash = 0;
    }
  ...
  const LIGHT_PATTERNS = [
    ...
    { id: 'fireflies', name: 'Lucioles', bars: 4, make: () => [
      { kind: 'round', r: 84, ...pt(), speed: 6, color: '#9fd8ff', jump: true }, { kind: 'round', r: 84, ...pt(), speed: 6, color: '#ffd166', jump: true }] },
  ...
  function nextPattern(c) {
    ...
    c.patId = p.id; c.patName = p.name; c.patBars = p.bars; c.beams = p.make(); c.barAt = Math.floor(Beat.index() / 4); c.fade = 0;
    for (const b of c.beams) { if (b.tx === undefined) { b.tx = b.x; b.ty = b.y; } }
    ...
    if (c.id === 'capture') {
      ...
      if (c.phase === 'lure') {
        ...
        if (dist(pl.x, pl.y, c.lure.x, c.lure.y) < c.lure.r * 0.8) {   // le joueur a rejoint la lumière : le spectacle commence
          c.phase = 'show'; nextPattern(c); AudioEngine.bossPhase({ intensity: 0.6 }); UI.banner('Les projecteurs s\'allument', c.def.color, 'Un seul motif de lumière à la fois, il change toutes les quelques mesures. Tuer dans la lumière : +50 % d\'XP.');
        }
      } else {
        ...
        for (const b of c.beams) {
          ...
          /* balayage : traversée régulière de gauche à droite, puis retour au bord */
          if (b.sweep) { b.x += b.sweep * 150 * dt; if (b.x > ROOM_X + ROOM_W - 30) b.x = ROOM_X + 30; b.tx = b.x; continue; }
          if (bar || (beat && b.jump) || (beat && RNG.chance(0.25))) {
  ...
  function lightMask(ctx, c, pl) {
    ...
    g.fillStyle = 'rgba(2,3,8,.985)'; g.fillRect(0, 0, w, h);   // noir presque total : dans l'ombre on ne devine plus les silhouettes
    ...
    const ph = Beat.phase(); const pulse = (1 + 0.12 * Math.max(0, 1 - ph * 2.5)) * (c.phase === 'lure' ? 1 : 0.4 + 0.6 * c.fade);
    ...
    if (c.phase === 'lure') { disc(c.lure.x, c.lure.y, c.lure.r * (1 + 0.08 * Math.sin(Time.now * 3))); }
      ...
      if (b.kind === 'round') disc(b.x, b.y, b.r * pulse);
        ...
        if (c.flash > 0) { g.globalAlpha = c.flash; g.fillStyle = 'rgba(0,0,0,1)'; g.fillRect(ROOM_X - 60, ROOM_Y - 60, ROOM_W + 120, ROOM_H + 120); g.globalAlpha = 1; }
    ...
    if (c.phase !== 'lure') {
      ...
      for (const b of c.beams) {
        ...
        else if (b.kind === 'strobe') { if (c.flash > 0) { ctx.globalAlpha = 0.13 * c.flash; ctx.fillRect(ROOM_X, ROOM_Y, ROOM_W, ROOM_H); ctx.globalAlpha = 0.13; } }
  ...
  function lit(c, x, y) {
    ...
    for (const b of c.beams) {
      if (b.kind === 'strobe') { if (c.flash > 0.2) return true; continue; }
  ...
  function renderOverlay(ctx, room) {
    ...
    if (!pl.dead && !lit(c, pl.x, pl.y)) {
      ...
      ctx.fillStyle = '#fff'; ctx.shadowColor = '#cfe8ff'; ctx.shadowBlur = 10;
      ctx.beginPath(); ctx.arc(pl.x - 4 * f, pl.y - 6, 2.4, 0, TAU); ctx.arc(pl.x + 4 * f, pl.y - 6, 2.4, 0, TAU); ctx.fill();
    }
    for (const p of Projectiles.list) { if (p.owner !== 'player') continue; ctx.fillStyle = p.color || '#fff'; ctx.shadowColor = ctx.fillStyle; ctx.shadowBlur = 10; ctx.beginPath(); ctx.arc(p.x, p.y, Math.max(2, p.r * 0.8), 0, TAU); ctx.fill(); }
    /* yeux des ennemis */
  ...
  function goal(room) {
    ...
    if (c.id === 'lights') { if (c.phase === 'lure') return c.lure; const b = c.beams.find(q => q.kind === 'round') || c.beams.find(q => q.kind === 'rect'); return b ? { x: b.kind === 'rect' && b.axis === 'h' ? G.player.x : b.x, y: b.kind === 'rect' && b.axis === 'v' ? G.player.y : b.y } : null; }
