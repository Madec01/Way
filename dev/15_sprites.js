...
const Music = (() => {
  /* Pistes fournies (CC-BY), utilisées par défaut pour tous les biomes.
     VOS musiques : déposez dans assets/music/custom/ les fichiers hub.mp3, biome1.mp3 (salles 1-4 et 6-8 du biome 1), boss1.mp3 (salles 5 et 9),
  ...
  function keyFor(kind) { if (kind === 'hub') return 'hub'; const n = (G.run && G.run.biome && G.run.biome.order) || 1; return kind + n; }
  ...
  function play(kind) {
    ...
    resolve(key).then(url => {
      ...
      if (p && p.then) p.then(ok => { if (!ok && current === key) { generative = true; AudioEngine.startGenerativeMusic(key.startsWith('boss') ? 'boss' : key === 'hub' ? 'hub' : 'biome'); } });
    });
  }
