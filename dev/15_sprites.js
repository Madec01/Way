...
/* ---- Chantier 12, séance B : un sol et des murs par palier ----
   Les quatre paliers partageaient les mêmes dalles 0x72 sous une teinte. Chaque palier tire maintenant son sol et ses
   murs d'une planche libre (CC0) : Kenney « Roguelike Caves & Dungeons » (pas de 17 px) et Ninja Adventure de
   Pixel-boy & AAA (pas de 16 px). Une tuile est [x, y, planche] ; sans troisième élément, c'est la planche 0x72.
   Le mur du haut reçoit deux à quatre motifs 0x72 (bouches d'aération, cuve qui coule, tentures) tirés du floorSeed,
   et une fontaine murale animée là où le palier en a une. */
const SHEETS = {
  kd: 'sprites/kenney_roguelike-dungeon/roguelikeDungeon_transparent.png',
  na_floor: 'sprites/ninja_adventure/TilesetFloor.png',
  na_interior: 'sprites/ninja_adventure/TilesetInteriorFloor.png',
  na_desert: 'sprites/ninja_adventure/TilesetDesert.png',
};
const kd = (c, r) => [c * 17, r * 17, 'kd'];
const na = (sheet, c, r) => [c * 16, r * 16, sheet];
const TILESETS = {
  /* l'hôpital : dalles grises fissurées, murs de pierre grise ; bouches d'aération et une fontaine rouge (le fluide des cuves) */
  biome_1: {
    floor: [kd(8, 2), kd(9, 2), kd(9, 3), kd(10, 3), kd(11, 3), kd(13, 3)],
    plain: 0.82,
    wallFace: kd(8, 1),
    wallTop: kd(8, 0),
    wallSide: kd(8, 1),
    decor: [TILES.hole, [48, 48], TILES.hole],
    fountain: { mid: [64, 16], frames: 3 },
  },
  /* la serre : terre brune tachée d'herbe, briques brunes ; une cuve qui coule et une fontaine bleue */
  biome_2: {
    floor: [
      na('na_floor', 1, 8),
      na('na_floor', 1, 11),
    ...
    wallTop: kd(17, 5),
    wallSide: kd(17, 6),
    decor: [TILES.goo, TILES.goo, [16, 48]],
    fountain: { mid: [64, 48], frames: 3 },
  },
  /* la Concession : sable beige, pierre brune ; des trous dans le mur */
  biome_3: {
    floor: [kd(17, 15), kd(16, 14), kd(19, 14), kd(18, 16), kd(20, 16), kd(16, 16)],
    plain: 0.8,
    wallFace: kd(17, 8),
    wallTop: kd(17, 7),
    wallSide: kd(17, 8),
    decor: [TILES.hole, [48, 48], [32, 48]],
    fountain: null,
  },
  /* le Sérail : mosaïque beige, grès clair ; des tentures et une fontaine bleue */
  biome_4: {
    floor: [
      na('na_interior', 1, 13),
      na('na_interior', 5, 13),
    ...
    plain: 0.84,
    wallFace: na('na_desert', 17, 1),
    wallTop: na('na_desert', 17, 0),
    wallSide: na('na_desert', 17, 1),
    decor: [[16, 32], TILES.banner, [32, 48], [16, 48]],
    fountain: { mid: [64, 48], frames: 3 },
  },
};
...
const Sprites = (() => {
  ...
  const palSheets = {}; // les planches des paliers (chantier 12), par clé de SHEETS
  function loadSheet(key) {
    return new Promise(res => {
      ...
      img.onerror = () => {
        console.warn('[Sprites] planche de palier indisponible : ' + key + ' (sol 0x72 en repli)');
        res(false);
      };
      img.src = ASSET_BASE + SHEETS[key];
    });
  }
  function load() {
    const paliers = Promise.all(Object.keys(SHEETS).map(loadSheet));
    return new Promise(res => {
      sheet = new Image();
      sheet.onload = () => {
        ready = true;
        paliers.then(() => res(true));
      };
      sheet.onerror = () => {
        failed = true;
  ...
  const tilesetOf = () => (G.run && G.run.biome && TILESETS[G.run.biome.id]) || null;
  ...
  const PROP_DEFS = {
    ...
    /* chantier 13 C : les corps des pièges (Kenney Industrial / Pixel Platformer / Tiny Dungeon, Ninja Adventure, CC0) */
    'oxygen-tank': { d: 'pixel', png: true },
    'beacon-red': { d: 'pixel', png: true },
    'pressure-plate': { d: 'pixel', png: true },
    'pressure-plate-down': { d: 'pixel', png: true },
    lever: { d: 'pixel', png: true },
    'oil-puddle': { d: 'pixel', png: true },
    'seed-pod': { d: 'pixel', png: true },
    'wire-post': { d: 'pixel', png: true },
    gargoyle: { d: 'pixel', png: true },
    'gargoyle-spit': { d: 'pixel', png: true },
    'floor-spikes-0': { d: 'pixel', png: true },
    'floor-spikes-1': { d: 'pixel', png: true },
    'floor-spikes-2': { d: 'pixel', png: true },
    'floor-spikes-3': { d: 'pixel', png: true },
  ...
  function drawWallFx(ctx, room) {
    const ts = tilesetOf();
    if (!ts || !ts.fountain || room.fountainX == null) return;
    const f = Math.floor(Time.now * 4) % ts.fountain.frames;
    tile(ctx, [ts.fountain.mid[0] + 16 * f, ts.fountain.mid[1]], room.fountainX, ROOM_Y - TILE);
  }
  ...
  function tile(ctx, t, dx, dy, w = TILE, h = TILE) {
    const img = t[2] ? palSheets[t[2]] : ready ? sheet : null;
    if (!img) return false;
    ctx.drawImage(img, t[0], t[1], 16, 16, dx, dy, w, h);
    return true;
  }
  ...
  function drawFloor(ctx, room) {
    ...
    if (!c) {
      ...
      /* le palier a sa planche ; si elle n'est pas chargée (file://, réseau), les dalles 0x72 reprennent */
      const ts0 = tilesetOf();
      const ts = ts0 && tile(g, ts0.floor[0], -TILE, -TILE) ? ts0 : null;
      g.fillStyle = '#07080d';
      g.fillRect(0, 0, W, H);
      ...
      /* murs */
      const wFace = ts ? ts.wallFace : TILES.wallFace,
        wTop = ts ? ts.wallTop : TILES.wallTop,
        wLeft = ts ? ts.wallSide : TILES.wallLeft,
        wRight = ts ? ts.wallSide : TILES.wallRight;
      for (let tx = -1; tx <= ROOM_COLS; tx++) {
        const x = ROOM_X + tx * TILE;
        if (!tile(g, wFace, x, ROOM_Y - TILE)) {
          g.fillStyle = '#232a44';
          g.fillRect(x, ROOM_Y - TILE, TILE, TILE);
        }
        if (!tile(g, wTop, x, ROOM_Y + ROOM_H)) {
          g.fillStyle = '#1a2036';
      ...
      for (let ty = 0; ty < ROOM_ROWS; ty++) {
        ...
        if (!tile(g, wLeft, ROOM_X - TILE, y)) {
          g.fillStyle = '#1a2036';
          g.fillRect(ROOM_X - TILE, y, TILE, TILE);
        }
        if (!tile(g, wRight, ROOM_X + ROOM_W, y)) {
          g.fillStyle = '#1a2036';
          g.fillRect(ROOM_X + ROOM_W, y, TILE, TILE);
        }
      }
      /* le mur du haut raconte le palier : deux à quatre motifs 0x72 (bouches, cuve qui coule, tentures) et la place
         de la fontaine animée, tirés du floorSeed, jamais dans les trois colonnes du coin ni sur un néon */
      room.wallDeco = [];
      room.fountainX = null;
      if (ts) {
        const n = 2 + Math.floor(rng() * 3);
        const cols = [];
        for (let k = 0; k < 40 && cols.length < n + (ts.fountain ? 1 : 0); k++) {
          const c = 2 + Math.floor(rng() * (ROOM_COLS - 4));
          if (cols.some(o => Math.abs(o - c) < 2) || (c + 0.5) % (ROOM_COLS / 5) < 1.2) continue;
          cols.push(c);
        }
        cols.forEach((c, i) => {
          const x = ROOM_X + c * TILE;
          if (ts.fountain && i === cols.length - 1) room.fountainX = x;
          else {
            const t = rng.pick(ts.decor);
            tile(g, t, x, ROOM_Y - TILE);
            room.wallDeco.push({ x, t });
          }
        });
      }
