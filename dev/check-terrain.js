...
for (const f of ['content.js', 'content2.js', 'content3.js', 'content4.js']) {
  ...
  for (const m of txt.matchAll(/terrain:\s*\[([\s\S]*?)\]/g)) {
    ...
    const before = txt.slice(0, m.index); const ids = [...before.matchAll(/id:\s*'([^']+)'/g)];
    const id = ids.length ? ids[ids.length - 1][1] : '(salle inconnue)';
    const lines = [...m[1].matchAll(/'([^']*)'/g)].map(x => x[1]);
    plans++;
    const fail = msg => { console.log(`  ✗ ${id} : ${msg}`); errors++; };
    ...
    if (solid(1, sy)) { fail('la tuile d\'entrée (1,' + sy + ') est bloquée'); continue; }
    const seen = new Set([`1,${sy}`]); const q = [[1, sy]]; let ok = false;
    while (q.length) {
      const [x, y] = q.shift();
      if (x === COLS - 1 && y === sy) { ok = true; break; }
