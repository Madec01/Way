...
if (!src || !fs.existsSync(src)) { console.error('Usage : node dev/install-sprites.js <dossier contenant les .zip>'); process.exit(1); }
...
const nextSlot = name => { if (!fs.existsSync(path.join(dest, name + '.png'))) return name; for (let i = 2; i < 50; i++) { const n = `${name}_v${i}`; if (!fs.existsSync(path.join(dest, n + '.png'))) return n; } return null; };

const zips = fs.readdirSync(src).filter(f => f.toLowerCase().endsWith('.zip')).sort();
if (!zips.length) { console.error('Aucun .zip dans ' + src); process.exit(1); }
...
for (const z of zips) {
  ...
  try { execFileSync('unzip', ['-q', '-o', path.join(src, z), '-d', out]); } catch (e) { skipped.push(z + ' (zip illisible)'); continue; }
  const files = walk(out);
  const meta = files.find(f => path.basename(f) === 'metadata.json');
  const png = files.find(f => f.toLowerCase().endsWith('.png'));
  if (!png) { skipped.push(z + ' (aucun PNG)'); continue; }
  let name = null;
  if (meta) { try { const j = JSON.parse(fs.readFileSync(meta, 'utf8')); const p = (j.objects && j.objects[0] && j.objects[0].prompt) || ''; name = p.split(/[\s\t]/)[0].replace(/\.png$/i, '') || null; } catch (e) { /* metadata illisible : on retombe sur le nom du zip */ } }
  if (!known.has(name)) { skipped.push(`${z} (accessoire « ${name} » inconnu — mettre le nom exact au début du prompt)`); continue; }
  const slot = nextSlot(name);
  if (!slot) { skipped.push(z + ' (plus de place)'); continue; }
