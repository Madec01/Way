...
const Pickups = {
  ...
  render(ctx) {
    for (const p of this.list) {
      ...
      else if (p.kind === 'pet') { const pd = Content.pet(p.pet) || {}; ctx.save(); ctx.globalAlpha = 0.9; if (!Sprites.drawProp(ctx, Sprites.pickDir(pd.sprite, 's'), p.x, p.y + bob - 4, 26, 26, {})) { ctx.fillStyle = pd.color || '#9fd8ff'; ctx.beginPath(); ctx.arc(p.x, p.y + bob - 4, 8, 0, TAU); ctx.fill(); } ctx.restore(); ctx.fillStyle = pd.color || '#9fd8ff'; ctx.font = '11px "Segoe UI", sans-serif'; ctx.textAlign = 'center'; ctx.fillText(pd.name || 'Compagnon', p.x, p.y + bob - 22); }
...
const Combat = {
  ...
  collect(p) {
    ...
    else if (p.kind === 'coin') { G.run.coinsPending += p.value; G.run.stats.coins += p.value; AudioEngine.pickupCoin({ step: streak }); }
...
const Weapons = {
  ...
  update(pl, dt, firing, aim) {
    const w = pl.weapon; const st = pl.stats; if (!w) return;   // écran de préparation : pas encore d'arme
...
class Player {
  ...
  render(ctx) {
    ...
    Sprites.drawBody(ctx, this.char && this.char.sprite || 'player', this.x, this.y, { face: this.char && this.char.face, body: this.char && this.char.body, size: this.char && this.char.size, anim: this.char && this.char.anim, clip: this.clip, clipT: this.clipT, dir: dv && dv.dir, flip: dv ? dv.flip : this.facing < 0, walk: this.walkT, flash: this.hurtFlash > 0, scale: G.room && G.room.tempo && G.room.tempo.started ? 1 + 0.07 * Math.max(0, 1 - Beat.phase() * 3) : 1, fallback: () => {
