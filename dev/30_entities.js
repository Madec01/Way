...
class Player {
  ...
  render(ctx) {
    ...
    Sprites.drawBody(ctx, this.char && this.char.sprite || 'player', this.x, this.y, { face: this.char && this.char.face, body: this.char && this.char.body, size: this.char && this.char.size, tier, flip: this.facing < 0, walk: this.walkT, flash: this.hurtFlash > 0, scale: G.room && G.room.tempo && G.room.tempo.started ? 1 + 0.07 * Math.max(0, 1 - Beat.phase() * 3) : 1, fallback: () => {
