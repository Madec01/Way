...
const Debug = (() => {
  ...
  function init() {
    ...
    panel.innerHTML = `
      ...
      <div class="drow"><label>Compagnon <select id="d-pet">${Content.pets().map(p => `<option value="${p.id}">${p.name}</option>`).join('')}</select></label><button class="btn small" id="d-givepet">Adopter</button><button class="btn small ghost" id="d-nopet">Renvoyer</button></div>
      <div class="drow"><label>Objet <select id="d-pick"><option value="purse">Bourse</option><option value="weapon">Arme d'essai</option><option value="ally">Allié</option><option value="relic">Relique</option><option value="heart">Cœur</option></select></label><button class="btn small" id="d-drop">Poser</button><label>Tenue <select id="d-tier"><option value="">auto</option><option value="0">nu</option><option value="1">vêtements</option><option value="2">armure</option><option value="3">chevalier</option></select></label></div>
    ...
    $('#d-givepet').onclick = () => { if (G.run) Pets.give($('#d-pet').value); };
