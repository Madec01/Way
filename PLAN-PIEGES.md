...
- **Séance A — le socle** ✔ (CONTENT.md §72) : les tables déclencheur / corps / effet avec `TRAP_LEGACY` (comportement identique), les dettes payées (`syncBeat` en cache, `slow` et `lifetime` lus, la grille et le dégradé du gaz calculés une fois, l'annonce en `PAL.alert`, la nappe dans la couleur du piège, la cadence des émetteurs musicale), le test `pieges.js` rejoué sur l'ancien moteur, le chemin sûr prouvé dans `salles.js` dans le temps et l'espace. Le joueur ne voit rien changer, sauf l'annonce des dalles et du gaz dans la couleur d'alerte et les nappes à la couleur de leur palier. Le type de salle `TRAP` est gardé : c'est celui de l'atelier.
- **Séance B — les deux camps** ✔ (CONTENT.md §73) : `BALANCE.trap` (ennemis ×1,5, boss ×0,25, compagnons ×0,5), les balles de piège qui abattent ce qu'elles croisent, les compagnons touchés à travers `Pets.hurt` (jamais le compagnon parti ni sonné), une recharge par cible, le banc `normal.js` avant et après ; les huit pièges dormants posés. Le joueur voit : une Nuée qui traverse un tourniquet meurt, une tourelle abat ce qui passe devant.
- **Séance C — le joueur décide** ✔ (CONTENT.md §74) : les déclencheurs `press`, `near`, `bullet`, `ignite` et le lien, les effets `push`, `status`, `burn`, `blast`, `arm`, `call`, le danger par déclencheur, `hp` et le boîtier (casser, déclencher, basculer, aiguiller), le corps `prop` en sprite, `KIND` de l'atelier étendu ; les seize pièges de la section 6 écrits et posés. *Écart* : le néon à bascule est la grille du tempo avec un boîtier, pas une définition à part ; les statuts n'ont pas de prise sur les compagnons.
- **Séance D — ce qui bouge et ce qui reste** ✔ (CONTENT.md §75) : le piège porté (`Room.dropTrap`, `behavior.drop`), `parent` sur module mobile, le sablier de salle (`room.trapShift`), la cloche et le coffre gardé, les sprites des dalles et des bouches, la cage dessinée, la galerie des 36 salles. *Écarté* : le calque de terrain temporaire (`room.gridFx`) — les zones au sol suffisent au feu qui reste.
