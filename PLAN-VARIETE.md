...
- **Un plan ASCII par salle** (13 lignes × 24, majuscule = obstacle typé, minuscule = déco, caractères du terrain inchangés) avec une légende par palier — *écarté à la séance D* : les plans de terrain vivent dans chaque salle à côté de ses obstacles et de son décor, et `check-terrain.js` lit les deux (obstacles comptés solides, règle du couloir de deux tuiles) ; un second format aurait doublé la source de vérité.
- **Un test `salles.js`** ✔ (séance D) : pour chaque salle de chaque palier, la porte est atteignable depuis le départ avec un chemin de largeur 2, rien dans le couloir de la porte ni sur l'entrée, un carré 3×3 libre là où le boss apparaît, le décor sur du sol nu, ≤ 40 rectangles ; `spawncheck.js` garde les apparitions. La **galerie** est un script de session (36 captures, quatre planches), livrée à chaque séance.
